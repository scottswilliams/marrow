...
use marrow_image::{
    ...
    RootOccurrenceDef, Scalar, SpanEntry, TypeId, ValueShapeLeaf, ValueShapeNodeId,
};
...
fn base_level(draft: &mut DraftTxn<'_>) -> Level {
    ...
    Level {
        shape: draft
            .value_struct(vec![
                ValueShapeLeaf::new("v", int),
                ValueShapeLeaf::new("w", text),
            ])
...
fn enclosing_level(draft: &mut DraftTxn<'_>, ordinal: usize, inner: Level) -> Level {
    ...
    Level {
        shape: draft
            .value_struct(vec![ValueShapeLeaf::new("inner", inner.shape)])
            .expect("a within-bounds shape appends"),
        ty: record_type(record),
    }
}
...
fn a_shared_value_shape_is_one_node_however_many_fields_reference_it() {
    ...
    for _ in 0..24 {
        level = values
            .struct_shape(vec![ValueShapeLeaf::new("inner", level)])
            .expect("the test arena mints");
    }
