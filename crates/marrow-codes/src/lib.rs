...
codes! {
    ...
    CheckRequiresPresence => r#"check.requires_presence"#, Check, Active, r#"A field or group write has no live entry-presence proof, or a required field/group-leaf read uses a proof invalidated within its lexical scope. Untested reads remain optional. Bind `place m = ^root[key]` and establish a proof with `if exists(m)`, `if const x = m`, a diverging negative guard or let-else, or whole-entry assignment through `m`. A proof ends at lexical exit, at an entry erase in its exact family, or at a call that erases that family. A protected use inside a loop entered after the proof also requires the proof to survive the repeating region. Recheck an invalidated place before using it. To create an entry, assign the whole record. See [named places](language/durable-places.md#named-places)."#;
    ...
    StoreApplyUnsupported => r#"store.apply_unsupported"#, Store, Active, r#"Explicit apply requires unchanged old durable representations and permits only sparse scalar field additions. The selected change is outside that scope; no store metadata was published."#;
