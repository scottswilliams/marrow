...
codes! {
    ...
    StoreAuditOutsideSchema => r#"store.audit_outside_schema"#, Store, Active, r#"`marrow doctor` found a well-formed cell that belongs to no declared root, field, group, branch, or index of the active program. The finding names the raw cell."#;
    ...
    StoreAuditIndexOrphan => r#"store.audit_index_orphan"#, Store, Active, r#"`marrow doctor` found an index row whose source entry does not exist. The finding names the index by its ledger identity and the row's projected values."#;
    StoreAuditIndexStale => r#"store.audit_index_stale"#, Store, Active, r#"`marrow doctor` found an index row whose projected values disagree with the source entry it names. The finding names the index and the row."#;
    StoreAuditIndexMissing => r#"store.audit_index_missing"#, Store, Active, r#"`marrow doctor` found a present entry whose projected values are complete but whose index row is absent. The finding names the index and the row the entry should have."#;
