...
codes! {
    ...
    CheckRequiresPresence => r#"check.requires_presence"#, Check, Active, r#"A field or group write addresses an entry no presence proof covers at the write. A write updates an entry the compiler has proved present: bind `place m = ^root[key]` and write through `m` inside `if exists(m) { … }` or `if const x = m { … }`, after `const x = m else { … }` or `if not exists(m) { return … }`, or after a whole-entry assignment `m = Resource(…)`. A proof ends at its block's end, at a `delete` of any entry in the same family, at a call whose demand writes the family, and — for a write inside a loop entered after the proof — when that loop body erases the family or calls a function that writes it. To create an entry, assign the whole record."#;
