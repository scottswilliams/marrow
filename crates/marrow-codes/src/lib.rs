...
codes! {
    ...
    CheckNameConflict => r#"check.name_conflict"#, Check, Active, r#"Two declarations share a name in one scope: two functions in one module, two declarations with one identifier, or a member, parameter, type parameter, or key column declared twice in one layer. The message names the owner and the repeated name."#;
    ...
    CheckTransactionEmpty => r#"check.transaction_empty"#, Check, Active, r#"An export owns a `transaction` block but performs no durable operation, directly or through a call, so it opens no store session. Remove the block or add the durable work the export is meant to perform. The requirement applies to the entire export: a read before an empty block satisfies it."#;
    CheckTransactionReopened => r#"check.transaction_reopened"#, Check, Active, r#"An export begins its `transaction` region more than once on some path: a second block after the first, a block inside another, or a block in a loop body that a later iteration begins again. An export begins its region at most once on any path and commits on every normal exit after begin. Combine the durable work into one block, or move the loop inside the block."#;
    CheckTransactionUncommitted => r#"check.transaction_uncommitted"#, Check, Active, r#"A path leaves its `transaction` block without committing: a `break` or `continue` that jumps out of the block, or a return while the region is still open. Every normal exit from an entered region commits its staged writes after evaluating the exit value and before returning; a loop that exits early belongs inside the block."#;
    CheckTransactionConditional => r#"check.transaction_conditional"#, Check, Active, r#"A `transaction` block runs on some paths through an export and not on others that meet them later, such as a block in one arm of an `if` or `match` whose arm continues past it. Paths that meet agree on whether the export's block has run. Return before the block when its work does not apply, or end the arm with `return` after the block. Reported at the block."#;
    CheckDurableAfterCommit => r#"check.durable_after_commit"#, Check, Active, r#"A durable read follows the commit of a `transaction` block on some path, directly or through a call. Move the read inside the block, or capture the value into a local before the block closes and return the local. A write or mutating call after the commit has no transaction and is `check.requires_transaction`."#;
    ...
    ImageFlow => r#"image.flow"#, Image, Active, r#"A program image failed transaction-flow verification (phase 5): a misplaced marker, an owner with no durable demand, an owner called outside a test, a write without an open region, durable access after commit, or inconsistent region state. Each path begins at most once and commits on every normal exit after begin; paths that meet agree on region state. Source counterparts include `check.transaction_*`, `check.requires_transaction` and `check.durable_after_commit`."#;
    ...
    RunCallDepth => r#"run.call_depth"#, Run, Active, r#"The call chain grew deeper than the fixed limit (64). A function cannot call itself, directly or through other functions, so this guards a very deep chain of distinct calls."#;
    ...
    StoreContractChanged => r#"store.contract_changed"#, Store, Active, r#"The supplied image does not match the binding required by this operation. Ordinary attachment refuses a changed durable contract or exported interface; explicit apply requires the exact OLD binding, and recovery requires the image named by the actual stored Head. This refusal does not change the binding or establish store integrity or service readiness. [Changing the program](operations/README.md#changing-the-program) describes the outcomes."#;
    StoreApplyUnsupported => r#"store.apply_unsupported"#, Store, Active, r#"Explicit apply publishes a new program only when every old durable representation is unchanged and the only additions are sparse scalar fields. The refusal's `reason` names the first change outside that scope; [`marrow apply`](tools/cli.md#marrow-apply) lists the reasons. Stored values are never converted. No store metadata was published."#;
    StoreCeilingUnaccepted => r#"store.ceiling_unaccepted"#, Store, Active, r#"Explicit apply requires acceptance of the exact union of the store's standing ceiling and the new image's demand when that union expands authority. A supplied identity must match even without expansion. No store metadata was published."#;
    StoreProvisionUnapproved => r#"store.provision_unapproved"#, Store, Internal, r#"Provisioning was refused because the presented approval was accepted for a different program image or destination spelling. The runner's `provision` command and `marrow import` accept the report they build for the same image and destination, so they do not reach this; it fails closed before any store is written. An approval records consent and does not authenticate."#;
    ...
    StoreImageNotActive => r#"store.image_not_active"#, Store, Active, r#"The program is a code-only edit of the store's active program, and the requested operation does not rebind. Import, doctor, recovery and backup require the active program. Present that program and retry. A deliberate code update through `marrow run --store` is a separate operation. The store is intact."#;
    StoreAuditUndecodable => r#"store.audit_undecodable"#, Store, Active, r#"`marrow doctor` found a cell whose key or value does not decode under the store's layout or the declared shape of its field. The finding names the field or, for a key the layout cannot place, the raw cell."#;
    StoreAuditOutsideSchema => r#"store.audit_outside_schema"#, Store, Active, r#"`marrow doctor` found a well-formed cell that belongs to no declared root, field, group, branch, or index of the active program. The finding names an undeclared index by its identity when possible, or the raw cell."#;
    StoreAuditRequiredMissing => r#"store.audit_required_missing"#, Store, Active, r#"`marrow doctor` found a present entry without one of its required fields. The finding names the entry and the field."#;
    StoreAuditOrphanLeaf => r#"store.audit_orphan_leaf"#, Store, Active, r#"`marrow doctor` found a field or group leaf stored under an entry that has no presence marker. The finding names the entry; a read of it faults `run.corruption`."#;
    StoreAuditMarkerInvalid => r#"store.audit_marker_invalid"#, Store, Active, r#"`marrow doctor` found an entry marker whose stored value is not the presence record. The finding names the entry."#;
    StoreAuditIndexOrphan => r#"store.audit_index_orphan"#, Store, Active, r#"`marrow doctor` found an index cell whose source entry does not exist. The finding names the index by its ledger identity and the projected values."#;
    StoreAuditIndexStale => r#"store.audit_index_stale"#, Store, Active, r#"`marrow doctor` found an index cell whose projected values disagree with the source entry it names. The finding names the index and the projected values."#;
    StoreAuditIndexMissing => r#"store.audit_index_missing"#, Store, Active, r#"`marrow doctor` found a present entry with complete projected values but no corresponding index cell naming it: the cell is absent or names another entry. The finding names the index and the expected projected values."#;
    StoreAuditWitnessInvalid => r#"store.audit_witness_invalid"#, Store, Active, r#"`marrow doctor` found the store's commit witness cell holding bytes no witness encoding this build reads. The next transaction against the store would refuse with `store.corruption`."#;
    IoRead => r#"io.read"#, Io, Active, r#"An operational read failed, such as reading a project source file, `marrow.toml`, the entropy source for a `marrow run` identity mint, a runner launch artifact, or a runner protocol frame."#;
    ...
    ProjectIdsMint => r#"project.ids_mint"#, Project, Active, r#"A storeless `marrow run` or `marrow test` could not mint missing identities: an anchor was invalid, duplicated, live, or retired; the ledger would exceed its fixed size; or a candidate id collided. `.marrow/ids` is unchanged. Fix the source or the ledger state, then run again; a collision may pass on another attempt."#;
