...
codes! {
    ...
    CheckTransactionReopened => r#"check.transaction_reopened"#, Check, Active, r#"A mutating export begins its `transaction` region more than once on some path: a second block after the first, a block inside another, or a block in a loop body that a later iteration begins again. An export owns one block, begins it at most once and commits on every normal exit after begin. Combine the durable work into one block, or move the loop inside the block."#;
