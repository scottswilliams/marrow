...
codes! {
    ...
    CheckInstantiationLimit => r#"check.instantiation_limit"#, Check, Error, NotApplicable, Active, r#"Monomorphizing a program requires more distinct generic instantiations than the fixed limit (4096). A well-typed program with an acyclic call graph mints finitely many instances; this bound (campaign law 9) fails a divergent monomorphization — typically a generic function that calls itself over an ever-growing type — with a typed error before the instantiation worklist allocates unboundedly."#;
