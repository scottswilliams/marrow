...
codes! {
    ...
    CheckTransactionConditional => r#"check.transaction_conditional"#, Check, Active, r#"A `transaction` block runs on some paths through a mutating export and not on others that meet them later, such as a block in one arm of an `if` or `match` whose arm continues past it. Paths that meet agree on whether the export's block has run. Return before the block when its work does not apply, or end the arm with `return` after the block. Reported at the block."#;
