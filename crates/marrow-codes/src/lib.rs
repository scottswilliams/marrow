...
codes! {
    ...
    StoreLimit => r#"store.limit"#, Store, Error, Active, r#"Marrow exhausted a fixed representational bound: a store framing length/count did not fit its `u32` field, a record/problem/index count overflowed, or the tagged `u128` commit-witness generation was exhausted."#;
    ...
    RunnerSpawn => r#"runner.spawn"#, Runner, Error, Active, r#"The terminal could not spawn the companion runner for a persistent run: the release-verified stock runner binary could not be executed. The store is untouched; no session was established."#;
}
