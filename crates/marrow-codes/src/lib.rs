...
codes! {
    ...
    StoreApplyUnsupported => r#"store.apply_unsupported"#, Store, Active, r#"Explicit apply publishes a new program only when every old durable representation is unchanged and the only additions are optional scalar fields. The refusal's `reason` names the first change outside that scope: `application`, `root`, `index`, `member_removed`, `member_changed`, `stored_value` (an old field's stored value would be read with a different meaning, including any reordered, renamed, added, removed or retyped field of a stored struct or enum payload), or `member_added`. Stored values are never converted. No store metadata was published."#;
