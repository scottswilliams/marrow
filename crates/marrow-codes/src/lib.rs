...
codes! {
    ...
    RunCommit => r#"run.commit"#, Run, Error, Active, r#"A durable transaction commit did not complete normally. A confirmed engine abort reports `incomplete` with `known_old` and leaves the store owner usable. An indeterminate engine result poisons that handle and produces one opaque affine recovery fact; while continuously holding the owner lock, the lifecycle closes the engine, freshly reopens the existing file at the retained path, audits it, and consumes the fact to classify the exact witness state as `known_old`, `known_new`, or `unknown`. Missing or invalid engine files are not created or adopted and classify as `unknown`. A known result preserves a usable freshly opened owner; `unknown` retires it and leaves the owner descriptor unclean. The owner lock does not prove engine-file identity against out-of-band substitution of a structurally valid store or prior snapshot; that remains outside the current guarantee. The invocation never resumes or fabricates a return value, and no case is retried automatically. The code is mapped to the transaction's source span and is not catchable inside the program."#;
    RunOutcomeUnknown => r#"run.outcome_unknown"#, Run, Error, Active, r#"A durable call was dispatched to the attached runner, but no complete reply could be read, so the call's outcome is unknowable from the caller's side: it may have run, wholly or partly. This includes every socket-read I/O failure after dispatch. The call is reported as outcome-unknown and is never automatically retried — a mutating call whose outcome is unknown must not run twice. Run a read-only export to observe the store's current state before deciding whether to act again. A reply that arrives but violates the wire grammar and an ordinary runtime fault remain distinct."#;
