...
codes! {
    ...
    CliDurableUnsupported => r#"cli.durable_unsupported"#, Cli, Active, r#"`marrow run` resolved an export that reads or writes durable data, and no store was given. The `marrow` process never opens a store. Run the export against a provisioned one: `marrow run <export> --store <dir>`. A storeless export is unaffected."#;
