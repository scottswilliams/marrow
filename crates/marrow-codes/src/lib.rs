...
codes! {
    ...
    CheckTransactionReopened => r#"check.transaction_reopened"#, Check, Active, r#"A mutating export opens a second `transaction` block. An export owns one block, begins it at most once and commits on every normal exit after begin. Combine the durable work into one block."#;
    CheckTransactionUncommitted => r#"check.transaction_uncommitted"#, Check, Active, r#"A path returns while its `transaction` region is still open. Every normal exit from an entered region must commit its staged writes after evaluating the exit value and before returning."#;
