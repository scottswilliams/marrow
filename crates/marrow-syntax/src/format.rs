...
/// durable shape, the text the activation fence's source digest hashes. Comments are prose
/// and blank lines are grouping; neither changes the schema a stored snapshot must satisfy,
/// so the declaration is rendered with all comment trivia removed and its grouping blanks
/// stripped, while every shape-bearing line — fields, types, keys, enum members, const
/// values, store paths — is preserved. Dropping comments from the tree before formatting,
/// rather than filtering rendered lines, reaches a trailing comment a line prefix cannot and
/// never mistakes a semicolon inside a string value for one.
pub fn durable_shape_rendering(source: &str, declaration: &Declaration) -> String {
    strip_layout_blanks(&format_declaration(source, &without_comments(declaration)))
}

/// A copy of the declaration with every comment and doc line removed, at every depth. The
/// formatter emits comments only from the tree's `docs` and `comments` fields, so clearing
/// them yields a rendering that carries schema shape and nothing else.
fn without_comments(declaration: &Declaration) -> Declaration {
    let mut declaration = declaration.clone();
    match &mut declaration {
        Declaration::Const(decl) => decl.docs.clear(),
        Declaration::Resource(decl) => {
            decl.docs.clear();
            decl.comments.clear();
            strip_resource_member_comments(&mut decl.members);
        }
        Declaration::Store(decl) => {
            decl.docs.clear();
            decl.comments.clear();
            for index in &mut decl.indexes {
                index.docs.clear();
            }
        }
        Declaration::Enum(decl) => {
            decl.docs.clear();
            decl.comments.clear();
            strip_enum_member_comments(&mut decl.members);
        }
        Declaration::Evolve(decl) => {
            decl.comments.clear();
        ...
        Declaration::Surface(decl) => decl.comments.clear(),
        Declaration::Function(decl) => {
            decl.docs.clear();
            for param in &mut decl.params {
                param.docs.clear();
            }
            strip_block_comments(&mut decl.body);
        }
    }
    declaration
}

fn strip_resource_member_comments(members: &mut [ResourceMember]) {
    for member in members {
        match member {
            ResourceMember::Field(field) => field.docs.clear(),
            ResourceMember::Group(group) => {
                group.docs.clear();
                group.comments.clear();
                strip_resource_member_comments(&mut group.members);
            }
        }
    }
}

fn strip_enum_member_comments(members: &mut [EnumMember]) {
    for member in members {
        member.docs.clear();
        member.comments.clear();
        strip_enum_member_comments(&mut member.members);
    }
}

fn strip_block_comments(block: &mut Block) {
    block.comments.clear();
    for statement in &mut block.statements {
        strip_statement_block_comments(statement);
    }
}

fn strip_statement_block_comments(statement: &mut Statement) {
    match statement {
        Statement::If {
            then_block,
            else_ifs,
            ...
            strip_block_comments(then_block);
            for else_if in else_ifs {
                strip_block_comments(&mut else_if.block);
            }
            if let Some(else_block) = else_block {
                strip_block_comments(else_block);
            }
        }
        Statement::IfConst {
            then_block,
        ...
        | Statement::For { body, .. }
        | Statement::Transaction { body, .. } => strip_block_comments(body),
        Statement::Try { body, catch, .. } => {
            strip_block_comments(body);
            if let Some(catch) = catch {
                strip_block_comments(&mut catch.block);
            }
        }
        Statement::Match { arms, .. } => {
            for arm in arms {
                strip_block_comments(&mut arm.block);
            }
        }
        Statement::Const { .. }
        | Statement::Var { .. }
...
#[cfg(test)]
mod durable_shape_tests {
    use crate::durable_shape_rendering;

    /// Render the durable shape of the source's single declaration, the text the
    /// activation fence's source digest hashes.
    fn shape(source: &str) -> String {
        ...
        let declaration = parsed.file.declarations.first().expect("one declaration");
        durable_shape_rendering(source, declaration)
    }

    /// A `;` line comment on a resource field — own-line or trailing — and a `;;`
    /// doc comment carry no schema shape, so none of them may drift the digest.
    #[test]
    fn resource_comments_do_not_change_shape() {
        let plain = "resource Book\n    title: string\n    pages: int\n";
        let own_line = "resource Book\n    ; a note\n    title: string\n    pages: int\n";
        let trailing = "resource Book\n    title: string ; the title\n    pages: int\n";
        let doc = "resource Book\n    ;; the title\n    title: string\n    pages: int\n";
        let group_comment =
            "resource Book\n    author\n        ; who wrote it\n        name: string ; full name\n";
        let group_plain = "resource Book\n    author\n        name: string\n";
        assert_eq!(shape(plain), shape(own_line));
        assert_eq!(shape(plain), shape(trailing));
        assert_eq!(shape(plain), shape(doc));
        assert_eq!(shape(group_plain), shape(group_comment));
    }

    #[test]
    fn store_comments_do_not_change_shape() {
        let plain = "store ^books: Book\n    index byTitle(title)\n";
        let commented =
            "store ^books: Book\n    ; the title index\n    index byTitle(title) ; unique-ish\n";
        assert_eq!(shape(plain), shape(commented));
    }

    #[test]
    fn enum_comments_do_not_change_shape() {
        let plain = "enum Status\n    draft\n    published\n";
        let commented = "enum Status\n    ; a work in progress\n    draft ; not yet live\n    ;; live\n    published\n";
        assert_eq!(shape(plain), shape(commented));
    }

    #[test]
    fn const_doc_does_not_change_shape() {
        let plain = "const limit: int = 10\n";
        let doc = ";; the ceiling\nconst limit: int = 10\n";
        assert_eq!(shape(plain), shape(doc));
    }

    /// A blank line is layout, not shape, so reformatting it away leaves the digest
    /// unchanged just as the comment edits do.
    #[test]
    fn blank_line_reformat_does_not_change_shape() {
        let dense = "resource Book\n    title: string\n    pages: int\n";
        let spaced = "resource Book\n    title: string\n\n    pages: int\n";
        assert_eq!(shape(dense), shape(spaced));
    }

    /// A real change to the schema — a field renamed, a type changed, an enum member
    /// added — must still drift the digest.
    #[test]
    fn shape_change_still_drifts() {
        let base = "resource Book\n    title: string\n    pages: int\n";
        let renamed = "resource Book\n    heading: string\n    pages: int\n";
        let retyped = "resource Book\n    title: text\n    pages: int\n";
        let added = "resource Book\n    title: string\n    pages: int\n    isbn: string\n";
        assert_ne!(shape(base), shape(renamed));
        assert_ne!(shape(base), shape(retyped));
        assert_ne!(shape(base), shape(added));
    }

    /// A semicolon inside a const string value is data, not a comment, so it must
    /// survive into the shape untouched.
    #[test]
    fn semicolon_in_const_string_is_preserved() {
        let one = "const greeting: string = \"a; b\"\n";
        let two = "const greeting: string = \"c; d\"\n";
        assert_ne!(shape(one), shape(two));
        assert!(shape(one).contains("\"a; b\""));
    }
}
