...
/// Find the sole diagnostic on the given 1-based source line, asserting its span
/// is a valid 1-based position rather than the start-of-input `0:0`.
fn sole_diagnostic_on_line(
    parsed: &marrow_syntax::ParsedSource,
    line: u32,
) -> &marrow_syntax::Diagnostic {
    let on_line: Vec<_> = parsed
        .diagnostics
        .iter()
    ...
    assert_eq!(
        on_line.len(),
        1,
        "exactly one diagnostic on line {line}: {:#?}",
        parsed.diagnostics
    );
    let diagnostic = on_line[0];
    assert!(
        diagnostic.span.line >= 1 && diagnostic.span.column >= 1,
        "the gap must carry a valid 1-based position, never `0:0`: {diagnostic:#?}"
    );
    diagnostic
}

#[test]
fn unclosed_paren_with_a_complete_operand_expects_the_close_paren() {
    // The operand `a` is complete; the missing `)` is the error, reported at the
    // gap rather than as a generic "expected a statement" at the `return` keyword.
    let parsed = parse_source("fn f()\n    return (a\n");
    let diagnostic = sole_diagnostic_on_line(&parsed, 2);
    assert_eq!(
        diagnostic.reason,
        parse_reason(ParseDiagnosticReason::Expected(ExpectedSyntax::CloseParen)),
        "{diagnostic:#?}"
    );
}

#[test]
fn call_missing_a_separator_expects_a_comma_or_close() {
    // `g(a b)` completes the argument `a` and then sees `b` where a `,` or `)`
    // belongs, so the gap names the separator rather than the statement keyword.
    let parsed = parse_source("fn f()\n    return g(a b)\n");
    let diagnostic = sole_diagnostic_on_line(&parsed, 2);
    assert_eq!(
        diagnostic.reason,
...
#[test]
fn unclosed_call_with_a_complete_argument_expects_a_comma_or_close() {
    // `g(a` ends with the argument complete and the list unterminated, the same
    // separator-or-close gap as a missing comma.
    let parsed = parse_source("fn f()\n    return g(a\n");
    let diagnostic = sole_diagnostic_on_line(&parsed, 2);
    assert_eq!(
        diagnostic.reason,
...
#[test]
fn empty_paren_still_expects_a_statement() {
    // An unterminated `(` with no operand is the R21-08 missing-operand case: the
    // gap report is suppressed inside the delimiter and the failure surfaces as
    // the statement-level "expected a statement", unchanged by the close-delimiter
    // diagnostic.
    let parsed = parse_source("fn f()\n    return (\n");
    let diagnostic = sole_diagnostic_on_line(&parsed, 2);
    assert_eq!(
...
#[test]
fn empty_call_still_expects_a_statement() {
    // `g(` with no argument is the R21-08 missing-operand case and likewise keeps
    // the statement-level fallback.
    let parsed = parse_source("fn f()\n    return g(\n");
    let diagnostic = sole_diagnostic_on_line(&parsed, 2);
    assert_eq!(
        diagnostic.reason,
...
/// Assert the parse produced exactly one diagnostic and that it is the `for`
/// header single-owner report, anchored at a valid 1-based position.
fn sole_for_header_diagnostic(parsed: &marrow_syntax::ParsedSource) {
    assert_eq!(
        parsed.diagnostics.len(),
        1,
        "the `for` header owns its single diagnostic; the close-delimiter gap \
         must not also fire inside the header: {:#?}",
        parsed.diagnostics
    );
    ...
    assert!(
        diagnostic.message.contains("for <binding> in <iterable>"),
        "the lone diagnostic is the `for` header message: {diagnostic:#?}"
    );
    assert!(
        diagnostic.span.line >= 1 && diagnostic.span.column >= 1,
        "the header diagnostic carries a valid 1-based position: {diagnostic:#?}"
    );
}

#[test]
fn for_header_unclosed_paren_iterable_reports_only_the_header() {
    // The for-header path owns the single diagnostic for a malformed iterable.
    // An unclosed `(` inside the iterable must not also raise the close-delimiter
    // gap, which would double-report against the body token on the next line.
    let parsed = parse_source("fn f()\n    for x in (a\n        x\n");
    sole_for_header_diagnostic(&parsed);
}

#[test]
fn for_header_call_missing_separator_in_step_reports_only_the_header() {
    // `by g(a b)` is a malformed step expression inside the for header; the
    // header diagnostic is the single owner and the separator gap stays silent.
    let parsed = parse_source("fn f()\n    for x in items by g(a b)\n        x\n");
    sole_for_header_diagnostic(&parsed);
}
