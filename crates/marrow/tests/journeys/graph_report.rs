...
//!
//! The fixture's text helpers live in the separate `graph_report_lib` project, which the
//! application reaches through its `[dependencies]` alias. The frozen report bytes and the
//! in-source test count are unchanged by that move: that is the transparency the two-tree
//! journey proves.
...
use crate::common::{MARROW_BIN, Project, conformance_dir, marrow_in};
