...
//!
//! Three frozen projects under `fixtures/v01/scale_corpus/`, each driven from source
...
fn clinical() -> Session {
    Project::from_fixture("scale_corpus/clinical").session()
}
...
fn erp() -> Session {
    Project::from_fixture("scale_corpus/erp").session()
}
...
fn m_traversal() -> Session {
    Project::from_fixture("scale_corpus/m_traversal").session()
}
