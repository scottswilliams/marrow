...
fn catalog_tests_all_pass_with_a_canonical_jsonl_surface() {
    let out = Project::from_fixture("tests_and_formatter/catalog")
...
fn catalog_text_report_summarizes_the_run() {
    let out =
        Project::from_fixture("tests_and_formatter/catalog").run_cli("catalog-text", &["test"]);
...
fn filter_selects_a_subset_and_keeps_the_discovered_total() {
    let workspace =
        Project::from_fixture("tests_and_formatter/catalog").materialize("catalog-filter");
...
fn outcomes_pin_passed_failed_and_errored_with_typed_codes() {
    let out = Project::from_fixture("tests_and_formatter/outcomes")
...
fn fmt_check_refuses_then_write_canonicalizes_idempotently() {
    let workspace =
        Project::from_fixture("tests_and_formatter/unformatted").materialize("fmt-unformatted");
...
fn unparseable_source_is_refused_and_left_untouched() {
    let workspace =
        Project::from_fixture("tests_and_formatter/unparseable").materialize("fmt-unparseable");
...
fn comment_loss_is_refused_and_left_untouched() {
    let workspace =
        Project::from_fixture("tests_and_formatter/comment_loss").materialize("fmt-comment-loss");
...
fn frozen_test_fixtures_are_formatter_canonical() {
    for name in ["catalog", "outcomes"] {
        let workspace = Project::from_fixture(&format!("tests_and_formatter/{name}"))
