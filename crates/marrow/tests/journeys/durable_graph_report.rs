...
//! text blob. This suite drives its *durable* analog — the `durable_graph_report` fixture —
...
fn session() -> Session {
    Project::from_fixture("durable_graph_report").session()
}
...
fn the_fixture_tests_pass_through_marrow_test() {
    let output = Project::from_fixture("durable_graph_report")
        .run_cli("durable-graph-report", &["test", "--format", "jsonl"]);
