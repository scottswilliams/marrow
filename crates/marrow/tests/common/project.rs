//! The in-process half of the `marrow` crate's fixture harness.
//!
//! A [`Project`] is an in-memory project — manifest, optional `.marrow/ids`, source
//! files — built inline or loaded from `crates/marrow/tests/fixtures/v01/<name>/`, and
//! driven through the library path ([`Project::image`], [`Project::try_image`],
//! [`Project::session`]). Assertions read typed outcomes, never rendered prose: a
//! [`CallOutcome`] fault carries the stable `marrow-codes` string and [`Diagnostics`]
//! carries `(code, line, column)`. A binary that never spawns the built binary includes
