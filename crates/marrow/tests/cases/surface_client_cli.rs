...
fn client_help_advertises_top_level_command() {
    ...
    let stdout = String::from_utf8(output.stdout).expect("stdout utf8");
    assert!(
        stdout.contains("marrow client typescript [--cursor-token] [--out <path>] <projectdir>")
    );
    assert!(
        stdout.contains("--cursor-token"),
        "client help must advertise the shipped cursor-token profile: {stdout}"
    );
    assert!(
        stdout.contains("--out"),
