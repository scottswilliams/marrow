...
// The fixture helpers of `empty_payload_marker_lifetime_agrees_across_engines`: the
// two payload shapes it writes, the committed-state checkpoint every step calls,
// and the operation sequence both engines replay.
fn empty() -> EntryValue {
    EntryValue {
        fields: Vec::new(),
        groups: Vec::new(),
...
fn replay_marker_lifetime<E: ByteEngine>(mut store: DurableStore<E>) {
    let root_keys = [key("parent")];
    let child_keys = [key("parent"), KeyScalar::Int(7)];
