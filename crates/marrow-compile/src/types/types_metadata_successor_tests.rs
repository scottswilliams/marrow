...
fn mint_ready(
    ...
    registry
        .mint_type_instance(draft, template, &[GArg::Scalar(ScalarType::Int)], site(3))
...
fn wrong_argument_counts_are_rejected_before_minting_owner_state() {
    ...
    for args in [
        ...
        let mut registry = test_registry(vec![struct_template("Pair", &["T", "U"])]);
        let mut draft_owner = ImageDraft::new();
        let mut draft = admitted(&mut draft_owner);
        let owner_before = owner_snapshot(&registry);
        let draft_before = draft_fingerprint(&draft);
        let result = registry.mint_type_instance(&mut draft, 0, &args, site(3));
        observations.push(
            result.is_err()
...
fn malformed_ready_metadata_is_hidden_from_semantic_readers() {
    ...
    let mut draft_owner = ImageDraft::new();
    let mut draft = admitted(&mut draft_owner);
    let missing_template = mint_ready(&mut registry, &mut draft, 0);
...
fn missing_template_ready_metadata_is_rejected_by_value_graph() {
    ...
    let mut draft_owner = ImageDraft::new();
    let mut draft = admitted(&mut draft_owner);
    let _ = mint_ready(&mut registry, &mut draft, 0);
    registry.generics.borrow_mut().type_insts[0].template = usize::MAX;
    let owner_before = owner_snapshot(&registry);
    let draft_before = draft_fingerprint(&draft);
...
fn malformed_reserved_option_ready_metadata_is_hidden_from_all_readers() {
    ...
    let option = registry
        .instantiate_reserved_option(&mut draft, GArg::Scalar(ScalarType::Int), site(3))
...
fn reserved_registry() -> (TypeRegistry, DraftTxn<'static>) {
...
fn target_case_with_registry(
    mut registry: TypeRegistry,
    mut draft: DraftTxn<'static>,
    target: GArg,
) -> bool {
    let owner_before = owner_snapshot(&registry);
    let draft_before = draft_fingerprint(&draft);
    let result = registry.mint_type_instance(&mut draft, 0, &[target], site(3));
    result.is_err()
        && owner_snapshot(&registry) == owner_before
        && draft_fingerprint(&draft) == draft_before
}

fn target_case(draft: DraftTxn<'static>, target: GArg) -> bool {
...
fn missing_non_scalar_targets_are_rejected_before_ready_publication() {
    ...
    let observations = vec![
        target_case(fresh_draft(), GArg::Nominal(NominalId(0))),
        ...
        target_case(fresh_draft(), GArg::Param(TypeParamIndex::from_position(0))),
    ];
...
fn wrong_family_record_targets_are_rejected_before_ready_publication() {
    ...
    let mut generic_registry = test_registry(vec![
        struct_template("Box", &["T"]),
        struct_template("Inner", &["T"]),
    ]);
    let mut generic_draft = fresh_draft();
    let generic = mint_ready(&mut generic_registry, &mut generic_draft, 1);
...
fn in_range_collections_recursively_validate_every_nested_target() {
    ...
    let mut list_draft = fresh_draft();
    ...
    let mut nested_draft = fresh_draft();
...
fn nested_non_ready_generic_targets_are_rejected_without_outer_publication() {
    ...
    for (is_enum, rejected) in [(false, false), (false, true), (true, false), (true, true)] {
        ...
        let mut registry = test_registry(vec![inner_template, struct_template("Outer", &["T"])]);
        let mut draft_owner = ImageDraft::new();
        let mut draft = admitted(&mut draft_owner);
        let inner = mint_ready(&mut registry, &mut draft, 0);
        ...
        let owner_before = owner_snapshot(&registry);
        let draft_before = draft_fingerprint(&draft);
        let result = registry.mint_type_instance(&mut draft, 1, &[inner_arg], site(3));
        observations.push(
            result.is_err()
                && owner_snapshot(&registry) == owner_before
                && draft_fingerprint(&draft) == draft_before,
...
fn unknown_value_target_is_not_an_ordinary_zero_edge_node() {
    ...
    let rejected =
        match registry.mint_type_instance(&mut draft, 0, &[GArg::Struct(orphan)], site(3)) {
            Err(_) => true,
...
fn valid_group_adds_no_value_containment_edge() {
    ...
    let outer = registry
        .mint_type_instance(&mut draft, 0, &[GArg::Group(group)], site(3))
        .expect("a real group is a valid non-containing argument");
    let graph = ValueGraph::build(&registry).expect("valid graph builds");
...
fn valid_collection_target_remains_accepted() {
    ...
    let id = registry
        .mint_type_instance(&mut draft, 0, &[GArg::Collection(collection)], site(3))
...
fn assert_exact_target_invariant(
    mut registry: TypeRegistry,
    mut draft: DraftTxn<'static>,
    target: GArg,
) {
    ...
    let draft_before = draft_fingerprint(&draft);
    let invariant =
        take_resolve_invariant(registry.mint_type_instance(&mut draft, 0, &[target], site(3)));
...
fn nested_non_ready_targets_report_the_exact_missing_body() {
    for rejected in [false, true] {
        ...
        let mut draft_owner = ImageDraft::new();
        let mut draft = admitted(&mut draft_owner);
        let inner = mint_ready(&mut registry, &mut draft, 1);
        let row = registry
            .generics
            .borrow()
            .type_insts
...
fn typed_struct_instance_owner_returns_only_a_ready_record() {
    ...
    let mut draft = admitted(&mut draft_owner);
    let record = registry
        .mint_struct_instance(&mut draft, 0, &[GArg::Scalar(ScalarType::Int)], site(3))
        .expect("valid struct instance is proven Ready");
    assert_eq!(record.index(), 0);

    let mut registry = test_registry(vec![enum_template("Maybe", "T")]);
    let mut draft_owner = ImageDraft::new();
    let mut draft = admitted(&mut draft_owner);
    let owner_before = owner_snapshot(&registry);
    let draft_before = draft_fingerprint(&draft);
