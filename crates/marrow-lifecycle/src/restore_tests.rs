//! Fresh-store restore from logical backup input: what is refused before the head
//! exists, and what a cut publication leaves behind.
...
fn fresh_populated_publication_prefix_recovers_without_replaying_construction() {
    ...
    directory.sync(Step::ConstructionStage).unwrap();
...
/// Change one published artifact under the retained descriptor, as a writer racing the
/// final admission would.
fn change_before_final_admission(directory: &AdmittedStoreDir, artifact: Artifact) {
...
fn final_admission_rejects_changed_metadata_and_retains_published_store() {
    for artifact in [Artifact::Envelope, Artifact::Head] {
        ...
        let error = restore_observed(
            &mut std::fs::File::open(backup).unwrap(),
            &destination,
            seam,
        )
        .unwrap_err();
...
fn publication_and_activation_barrier_failures_retain_destination_and_instance() {
    for activation in [false, true] {
        ...
        let error = restore_observed(
            &mut std::fs::File::open(artifact).unwrap(),
            &destination,
            seam,
        )
        .unwrap_err();
