...
/// Public durable-place vocabulary the reference must keep, so the rewrite
/// cannot pass by deleting the semantic core.
...
fn language_docs_use_public_vocabulary_only() {
    ...
    let storage_doc = std::fs::read_to_string(language_docs_dir().join("durable-places.md"))
        .expect("read durable-places doc")
        .to_lowercase();
    ...
    assert!(
        missing.is_empty(),
        "durable-places.md must state the public durable-data model, missing: {missing:#?}"
    );
}
...
fn documented_module_examples_check_clean() {
    ...
    for block in all_mw_blocks() {
        assert!(
            block.source.trim_start().starts_with("module "),
            "{} block {} is an unchecked fragment; use a complete module or a text fence",
            block.file_name,
            block.index
        );
