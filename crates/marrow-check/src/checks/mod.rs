...
//! type driver (`statements`), the loop-head classifier, frame, and diagnostics
//! (`loop_head`), saved-path shape predicates (`saved_paths`), range-for rules
//! (`ranges`), the operator/condition/assign/return/throw checks (`operators`),
//! saved-access key typing (`saved_keys`), straight-line required-field assignment
//! (`required_fields`), call checking (`calls`), the statically-known integer fold
//! for sequence positions (`const_int`), and the shared diagnostic constructors
//! (`diagnostics`).
