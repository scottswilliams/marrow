...
1. `lex_source` — text to a flat token stream. The lexer maintains an indent stack and an `open_delimiters` counter, synthesizing `INDENT`/`DEDENT`/`NEWLINE` layout tokens (not lexical characters) and suppressing them inside `(`/`[`. It caps the indent stack at `NESTING_DEPTH_LIMIT`: a line nesting past the limit opens no block, has its content dropped from the stream, and reports a single `check.nesting_limit`, so the token stream stays bounded however deep the source nests. The stream is lossless: token spans are in bounds, sorted by start, and non-overlapping, so walking the tokens and filling each inter-token gap (whitespace, blank lines, over-limit content) from the source reconstructs the file exactly — a round-trip property test (`tests/cases/roundtrip.rs`) enforces it, and it is what makes comment and layout trivia recoverable from the stream. Tabs and obsolete operators (`&&`, `||`, `!`, `#`) and the reserved `~` are lexer diagnostics.
...
`ParsedSource = { file: SourceFile, diagnostics }`. `SourceFile` holds the optional `module`, `uses`, ordered `declarations`, and file-level `comments`, with name-lookup accessors downstream crates use. The AST records no parentheses — the formatter re-derives the minimum from a precedence table that must stay in sync with `parse_expr.rs`. `TypeExpr` is the parsed structure of a type annotation — `sequence[T]`, `Id(^root)` (with the constructor, caret, and root spans recorded), a trailing `?` as a structural `Optional`, or otherwise a `Name` carrying the whitespace-free spelling. Classifying a name as a scalar, `unknown`, or a project-resolved type is left to `marrow-schema` and the checker; the parser owns the spelling grammar, and `Display` renders the node back to its whitespace-free source form for the formatter and the durable digest. `absent` is an ordinary primary expression (`Expression::Absent`), so a `return` of `absent` is a `Return` of that value. Comments are retained as file, declaration-body, evolve-body, and statement-block trivia with `placement` and column so `parse -> format` round-trips losslessly. `SourceSpan` (file-absolute byte range plus 1-based line/column) is on every token, node, and diagnostic.
