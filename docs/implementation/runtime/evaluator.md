...
| `crates/marrow-run/src/value.rs` | The `Value` model and its codecs: scalar/temporal/decimal/bytes/enum/sequence/local-tree/resource/identity, `IdentityValue` (root + keys), conversions to/from `SavedValue`/`SavedKey`/leaf bytes, text rendering. `Sequence` owns the 1-based sparse local-sequence invariant (populated positions in key order, holes allowed), so a positional write past the end and a delete leave holes exactly as a saved sequence does. `LocalTree` owns the keyed local-tree invariant as a `BTreeMap` from key tuple to value, giving key-ordered enumeration and `O(log n)` writes for any arrival order. |
