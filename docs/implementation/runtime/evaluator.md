...
- **Control flow.** `Flow` (Normal/Return/Break/Continue/Throw) is the result of a statement or block; `Flow::Return` carries an `Option<Value>`, where `None` is a void return or a returned empty optional. `eval_block` pushes/pops a scope balanced on every exit including faults; `loop_exec` maps innermost-loop Break/Continue to a `LoopStep`.
...
- **Compound assignment.** `statement.rs` evaluates `target op= value` by
  resolving and reading the target first, evaluating the right-hand expression
  through the shared binary-operator implementation, then writing the computed
  value back through the same resolved target. Saved paths use one `SavedPath`;
  local collection targets use one resolved position/key tuple.
...
| `crates/marrow-check/src/entry_abi.rs` | Checked entry ABI descriptors: builds `entry.invoke.v1` descriptor identities from public entry signatures over the single `EntryParameterShape` parameter carrier (presence read off the parameter type), return shape, accepted catalog identities, and the single `EntryResultShape` result carrier (presence read off the return type). The tag is a callable ABI tag, not a function-body digest. |
...
| `crates/marrow-run/src/expr.rs` | Pure-expression evaluation: literals, names/enum members, unary/binary operators with checked-overflow numeric and temporal arithmetic, string concatenation, comparison/equality, `is`, `??`, interpolation, field and optional-field dispatch; `eval_optional` is the one boundary that turns a `T?`-typed expression into an `Option<Value>`. |
| `crates/marrow-run/src/statement.rs` | Statement execution: const/var binding, assignment and compound-assignment dispatch (present-or-clear routes a `T?` saved assignment's absent arm to the node-delete planner), delete/return/throw/expr, control statements (if/match/break/continue/while/for/transaction/try); fires the before-statement hook. |
...
| `crates/marrow-run/src/value.rs` | The `Value` model and its codecs: scalar/temporal/decimal/bytes/enum/sequence/local-tree/resource/identity plus `Value::Absent`, the in-memory empty optional (never encoded to a store cell), `IdentityValue` (root + keys), conversions to/from `SavedValue`/`SavedKey`/leaf bytes, text rendering including bracketed sequences. `Sequence` owns the 1-based sparse local-sequence invariant as a `BTreeMap` from position to value (populated positions in key order, holes allowed), so a positional write past the end and a delete leave holes exactly as a saved sequence does, with `O(log n)` reads, writes, and deletes for any position. `LocalTree` owns the keyed local-tree invariant as a `BTreeMap` from key tuple to value, giving key-ordered enumeration and `O(log n)` writes for any arrival order. The validated `Position` (1-based, out-of-range unrepresentable) and `CollectionKey` newtypes are the addresses both kernels take, minted only in `collection/local.rs`. |
...
| `crates/marrow-run/src/path.rs` | Saved-path lowering: `lower` walks a checked place into a `SavedPath`; `SavedPath::read`/`write`, key lowering with identity-splice, typed-keyspace, and 1-based sequence-position guards, `lower_for_probe`/`saved_path_present` folding a non-positive position into absence for `??`/`exists`. |
| `crates/marrow-run/src/neighbor.rs` | `next`/`prev` ordered navigation: seeks the adjacent record or data-layer child via store cursors, returns an identity or key value, or the empty optional (`Value::Absent`) at a layer edge. A composite layer navigates the first column its key prefix leaves unfilled — a partial prefix seeks that inner column's edge entry, a fully-keyed leaf anchors a sibling seek in the final column under the preceding columns. |
...
Saved-write and read execution (`write*`, `read`, `durable_read`, `group_write`, `collection`, `saved_iter`), the stdlib boundary (`stdlib`, `std_pure`, `host_effects`), `transaction`, and `evolution` are sibling subsystems documented on their own pages.
...
- **Optionality is `Option<Value>` at the boundaries.** `eval_optional` evaluates a `T?`-typed expression to `Option<Value>`: the empty optional — the `absent` literal, or a `Value::Absent` from a missing sparse/keyed/positional read, a neighbor edge, a stdlib op, or a `T?`-returning call that returned absent — collapses to `None`, and a present value widens to `Some`. `??`, `if const`, `exists`, `?.`, `return`, materializing into a `T?` `const`/`var`/argument slot (`eval_into_slot`, absence becomes `Value::Absent`), and a present-or-clear saved assignment consume it.
- **A definite fault still propagates through the optional boundary.** `eval_call_expr` raises `run.no_value` only for a genuine void call, and a definite host op's `run.absent_element` (a missing required env var) is not collapsed to `None`, so a `catch` can bind it. A fixed-address required-data read and a dangling unique-index entry stay fatal `run.absent_element`.
