...
| `crates/marrow-check/src/entry_abi.rs` | Checked entry ABI descriptors: builds `entry.invoke.v1` descriptor identities from public entry signatures, parameter shapes, return shape, accepted catalog identities, and return presence. The tag is a callable ABI tag, not a function-body digest. |
...
| `crates/marrow-run/src/statement.rs` | Statement execution: const/var binding, assignment and compound-assignment dispatch, delete/return/throw/expr, control statements (if/match/break/continue/while/for/transaction/try); fires the before-statement hook. |
...
| `crates/marrow-run/src/neighbor.rs` | `next`/`prev` ordered navigation: seeks the adjacent record or data-layer child via store cursors, returns an identity or key value, or an absent fault at a layer edge. A composite layer navigates the first column its key prefix leaves unfilled — a partial prefix seeks that inner column's edge entry, a fully-keyed leaf anchors a sibling seek in the final column under the preceding columns. |
