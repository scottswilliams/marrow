...
- **Surface node reads** (`surface.rs`): admitted singleton and point reads over
  stable `SurfaceFact`s; each read validates the full backing record footprint
  required by the surface profile, then returns only the checked projection.
...
| `crates/marrow-run/src/surface.rs` | Transport-neutral surface node reads: store/catalog admission, fact-compiled projection plans, `surface.*` error mapping, singleton/point execution. |
