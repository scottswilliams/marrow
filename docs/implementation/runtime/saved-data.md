...
- **`shape_row` makes value reads lazy.** `read_value` runs only for Values/Entries shapes, so a Keys loop pays nothing to materialize the record it would otherwise read.
...
- **Read-site resolution probes before reading.** `??`, `if const`, and `exists` test the fixed saved address first and treat ordinary absence as control flow. A saved read that reaches `SavedPath::read` and still finds absence raises fatal `run.absent_element`; required-field materialization after an address is fixed is the same invalid attached-data fault. A local-collection indexed read, a sparse field of a materialized value, and a stdlib cell selection have no fixed saved address to probe, so they raise `absent_read`/`eval_local_field_get`'s catchable `run.absent_element` at the read site and the same `??`/`if const`/`exists`/`catch` forms resolve it; an unguarded one surfaces with that code.
- **A non-positive sequence position addresses no node on every read path.** Lowering a below-1 position raises the catchable absent fault, so a write surfaces it but a read resolves it. `count` and the `next`/`prev` neighbor seek lower through `lower_for_probe`: a folded-away position counts as 0 and seeks no neighbor, exactly as a positive out-of-range position does.
...
- **Local sequences are 1-based; `LocalTree` is a key-ordered map** so both enumerate in saved ascending order. `LocalTree` backs the keyed local tree with a `BTreeMap` from the full key tuple to its value, keeping insert/lookup/delete `O(log n)` for any key arrival order. Composite local-tree keys enumerate only the first column.
