...
| `evolution/auto_apply.rs` | Classifies the witness's heaviest record obligation (`RunObligation`: backfill, transform, populated rename re-address, destructive drop, repair) and applies only `ZeroMutation` via the production path, else `MustFence`. |
