...
- Native access fails closed, never crashes: opens, reads, writes, and the database close all run under one panic backstop (`catch_store_panic`, reached via `catch_open` for opens and `catch_traversal` for every `RedbStore` read/get/scan, write/delete transaction, and the `Drop` that closes the handle). redb reads btree pages lazily, so damage below the table roots opens cleanly and only panics — a layout assertion or slice-index `unreachable!()` — when a read traverses the tree, a write transaction descends it, or the close flushes redb's allocator and region bitmaps; `catch_traversal` maps that panic to `store.corruption` for every access shape (point get, forward and reverse scan, seek, snapshot pin, insert/remove, close) instead of aborting the process. Navigation that seeks the next/previous child from a prior key (record and data-path descents, index walks) holds a strict-monotonic-progress guard: a child that does not advance past the cursor is backend damage, surfaced as `store.corruption` rather than spun in an unbounded loop; the bounded-batch delete resumes each batch strictly past the prior one so a re-entered damaged page is seeked over, not looped on. `recover` attempts a write-capable repair and then proves it converged: it opens write-capable and walks the tree via `TreeStore::verify_readable` (which decodes and value-validates every data cell, rejects an out-of-order identity, and descends each `(store, record arity)` through the same seek-driven `for_each_record` navigation `data integrity` uses, so a damaged page that hides off the linear scan but on a record re-seek surfaces `store.corruption`), drops that handle so redb persists what the replay produced, and finally re-opens read-only and re-walks. Allocator- and slot-region damage can replay into a handle that reads cleanly yet leaves the file still demanding recovery on the next open; the fresh read-only re-open is the convergence oracle, so recover reports success only when the next read would succeed and otherwise reports `store.corruption` rather than a false repair. redb open errors map by damage (`map_open_error`): a torn body to corruption, an unclean-shutdown repair-needed file to the typed `store.recovery_required` (a write-capable open attempts the replay and reports whether the store opened), a read/write holder conflict to `store.locked`, a denied open to the path-bearing `store.permission_denied`, everything else to `store.io`.
