...
- Native access fails closed, never crashes: opens and reads run under one panic backstop (`catch_store_panic`, reached via `catch_open` for opens and `catch_read` for every `RedbStore` read, get, and scan). redb reads btree pages lazily, so damage below the table roots opens cleanly and only panics — a layout assertion or slice-index `unreachable!()` — when a read traverses the tree; `catch_read` maps that panic to `store.corruption` for every access shape (point get, forward and reverse scan, seek, snapshot pin) instead of aborting the process. `recover` opens then walks the tree via `TreeStore::verify_readable`, so a store it cannot make readable reports `store.corruption` rather than a false repair. redb open errors map by damage (`map_open_error`): a torn body to corruption, an unclean-shutdown repair-needed file to the typed `store.recovery_required` (a write-capable open attempts the replay and reports whether the store opened), a read/write holder conflict to `store.locked`, a denied open to the path-bearing `store.permission_denied`, everything else to `store.io`.
