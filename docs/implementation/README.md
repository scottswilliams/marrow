...
| `marrow-test-support` | The test scaffolding more than one binary builds over: the one scratch directory every test mints temporary files under, the image, ledger and project builders, the engine doubles, and the verifier's tracer corpus. A `dev-dependencies` edge only; it ships in nothing | [Contributing](../../CONTRIBUTING.md) |
