...
| `crates/marrow-check/src/entry_abi.rs` | Checker-owned callable ABI descriptors: resolves public entries to `entry.invoke.v1` identities over parameter shapes, result presence, result shape, and accepted catalog identities; owns the shared scalar, enum, identity, sequence, and computed-read resource result shape builder consumed by action and future computed-read surface descriptors. |
