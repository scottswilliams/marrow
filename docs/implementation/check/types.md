...
| `crates/marrow-check/src/surface_abi.rs` | Checker-owned surface ABI descriptors and digest framing: renders accepted-catalog read, sparse-update, and action operation descriptors for stable surfaces; computes the `surface.read.v1` / `surface.update.v1` operation tags consumed by runtime and JSON boundaries; and reuses `entry.invoke.v1` identity over parameters and return shape for actions. Shared value-shape classification lives here so read and update descriptors do not drift. |
