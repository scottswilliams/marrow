...
| `crates/marrow-check/src/entry_abi.rs` | Checker-owned entry invocation descriptors: resolves public entries to `entry.invoke.v1` identities over parameter shapes, accepted catalog identities, and return presence; runtime consumes these descriptors instead of reconstructing ABI ownership. |
| `crates/marrow-check/src/surface.rs` | Application-surface checker pass: resolves `surface` declarations to checker-valid store, top-level field, and index facts after catalog binding; records transport-neutral `SurfaceFact`s with derived `SurfaceReadOperationFact`s for node reads, collection pages, unique-index lookups, backing-record footprint, projection, sparse update fields, and source-only/stable catalog status; emits `check.surface_target` / `check.surface_field`. Parsed/resolved `create` items are reserved metadata and not part of the active serialized ABI profile. |
| `crates/marrow-check/src/surface_abi.rs` | Checker-owned surface ABI descriptors and digest framing: renders accepted-catalog read and sparse-update operation descriptors for stable surfaces and computes the `surface.read.v1` / `surface.update.v1` operation tags consumed by runtime and JSON boundaries. Shared value-shape classification lives here so read and update descriptors do not drift. |
