...
| `crates/marrow-check/src/model/` | The semantic model's id vocabulary and identity-recovery view: `model/ids.rs` owns the snapshot-scoped `u32` id newtypes (`ResourceId`, `EnumId`, `StoreId`, …) every fact and type leaf carries; `model/decls.rs` owns the `StoreRootArena` that interns identity roots and the `DeclIds` view a diagnostic renders an interned type leaf's spelling back through. |
...
| `crates/marrow-check/src/facts.rs` | Read-only typed fact tables (`CheckedFacts`) keyed by the `model/ids.rs` newtypes, `CheckedType`, `StoredValueMeaning` (durable-key decoding), presence proofs, effect summaries, and the catalog-id bridge that binds a stable durable id onto each of the six catalog-bound families. |
