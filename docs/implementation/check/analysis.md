...
| `crates/marrow-check/src/tooling/integrity.rs` | Integrity verdicts: per-value decode/key-type/enum-member checks, identity referent-existence verdicts, required-field completeness for existing records/keyed entries, orphan classification as typed `IntegrityProblem` with stable codes, and the store-completeness gate (`verify_store_completeness`) that runs the committed-lock root witness (`verify_store_roots_against_lock`, taking the live store as `Option` so an absent store presenting no roots is rejected just like a rolled-back one when the lock records committed roots), the store's per-root structural-digest cross-check, and the index-completeness cross-check (the entries each declared index owes derived from the data records), failing closed when a family enumerates a state its oracle disagrees with or the store dropped a committed root. |
