...
| `crates/marrow-check/src/tooling/integrity.rs` | Integrity verdicts: per-value decode/key-type/enum-member checks, identity referent-existence verdicts, required-field completeness for existing records/keyed entries, orphan classification as typed `IntegrityProblem` with stable codes, and the index-completeness cross-check (`verify_store_index_integrity`) that counts the entries each declared index owes from the data records and fails closed when the index family enumerates a different number. |
