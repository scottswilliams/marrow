...
- `tooling::source_saved_root_cursor_fact_at(snapshot, file, offset)` returns
  ...
  identity; catalog-backed definition and references use source catalog
  navigation facts when resolution succeeds.
...
| `crates/marrow-check/src/tooling/integrity.rs` | Integrity verdicts: per-value decode/key-type/enum-member checks, identity referent-existence verdicts, required-field completeness for existing records/keyed entries, orphan classification as typed `IntegrityProblem` with stable codes, and the store-completeness gate (`verify_store_completeness`) that runs the store's per-root structural-digest cross-check and the index-completeness cross-check (the entries each declared index owes derived from the data records), failing closed when a family enumerates a state its oracle disagrees with. The committed-lock root comparison (`verify_store_roots_against_lock`, taking a present live store and rejecting one missing a saved (`Store`) root whose lock-recorded activation epoch the store's own epoch covers — a root activated past the store's epoch is the store-behind case, not a loss) sits outside the completeness gate. Callers never reach it directly: the admission owner `verify_present_store_lock_roots` (in `marrow-run`'s `admission` module) is its sole caller and adds the present-vs-absent discrimination, treating an absent store body as the disposable-store case the write paths seed. |
