...
| `crates/marrow-check/src/tooling/integrity.rs` | Integrity verdicts: per-value decode/key-type/enum-member checks, identity referent-existence verdicts, required-field completeness for existing records/keyed entries, orphan classification as typed `IntegrityProblem` with stable codes, and the store-completeness gate (`verify_store_completeness`) that runs the store's per-root structural-digest cross-check and the index-completeness cross-check (the entries each declared index owes derived from the data records), failing closed when a family enumerates a state its oracle disagrees with. The committed-lock root witness (`verify_store_roots_against_lock`, taking a present live store and rejecting one presenting fewer saved (`Store`) roots than the lock records — it keys on the saved-root set alone, not the epoch, so a behind store missing a committed root is a loss whatever its epoch) sits outside the completeness gate: callers route through the single owner `verify_present_store_lock_roots`, which treats an absent store body as the disposable-store case the write paths seed. |
