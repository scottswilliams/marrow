...
| `crates/marrow/src/cmd_surface.rs`, `crates/marrow/src/cmd_surface/serve.rs` | `surface serve`: foreground loopback HTTP server over descriptor-derived surface routes and operation envelopes; default mode is read-only, while `--write` serves sparse-update/action routes through `ProjectSurfaceSession`. |
