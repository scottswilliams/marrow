...
| `crates/marrow/src/cmd_serve.rs` | `serve`: foreground loopback HTTP server over descriptor-derived surface routes and operation envelopes; default mode serves read and computed-read routes through `ProjectSurfaceReadSession`, while `--write` also serves create/update/delete/action routes through `ProjectSurfaceSession`. |
