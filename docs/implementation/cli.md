...
| `client typescript` | `cmd_client.rs`, `cmd_client/typescript.rs` | Parses the client generator command, checks the project against the committed catalog artifact without opening the store, reports ordinary check diagnostics on failure, and prints the self-contained TypeScript operation client on success. |
