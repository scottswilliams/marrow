...
- **Evolve apply.** `evolve apply` binds accepted identity from the live store snapshot, the sole write-time authority, falling back to the committed `marrow.lock` for first-run adoption when no store is present. It freezes a pending baseline into the store, then previews the witness. A Retire-bearing witness requires `--backup <path>` or `--no-backup`; the backup path must not resolve under managed project paths (`marrow.json`, `marrow.lock`, source roots, test paths, or the native data directory/store file), then uses the shared typed backup artifact writer and validates the archive before apply establishes any missing store UID or stages evolution work. The apply advances the catalog snapshot, the epoch, and the data in one store transaction; after commit, the CLI regenerates `marrow.lock` as a one-way projection of the committed store snapshot.
