...
`marrow run` and `marrow test` use `marrow-run::ProjectSession` for project checking, catalog binding, store admission, and entry invocation. The command modules parse flags, choose hooks, and render the resulting reports. For `run --arg`, `cmd_run.rs` is only the argv adapter: it preserves repeated `name=value` pairs in argv order and passes them to `marrow-run::entry::CheckedEntryCall`, where the checked signature owns textual decoding. For `run --format json`, `cmd_run.rs` owns the result envelope while `marrow-json` renders the return value when it belongs to the existing entry-return JSON surface. `marrow surface serve` uses `ProjectSurfaceReadSession` by default, `ProjectSurfaceSession` with `--write`, and validated `surface.route.v1` bindings over the `surface.operation.v1` DTOs to run the dependency-free loopback HTTP profile; the CLI owns the listener, HTTP subset, process lifetime, mode selection, optional exact loopback CORS origin, and strict request routing, while `marrow-json` owns ABI-derived operation catalog construction, route/ABI validation, JSON decode/execute/render for read, computed-read, create, update, delete, and action operations. `marrow surface client typescript` uses the check-style project path that reads only the committed catalog artifact, then asks `marrow-json` to render the TypeScript operation client from the same ABI and route manifest.
...
| `surface client typescript` | `cmd_surface.rs`, `cmd_surface/client.rs`, `cmd_surface/client/typescript.rs` | Parses the client generator subcommand, checks the project against the committed catalog artifact without opening the store, reports ordinary check diagnostics on failure, and prints the self-contained TypeScript operation client on success. |
| `surface serve` | `cmd_surface.rs`, `cmd_surface/serve.rs`, `cmd_surface/serve/cors.rs` | Parses the loopback bind target, optional `--write` mode, and optional exact loopback `--cors-origin`, opens `ProjectSurfaceReadSession` or `ProjectSurfaceSession`, builds ABI-derived operation catalog and validated route bindings from `surface.route.v1`, filters allowed routes by mode, accepts one bounded JSON `POST` per connection plus matching CORS preflight when configured, validates route/body operation agreement, dispatches through the matching surface operation executor, and returns sanitized JSON success/error envelopes. |
...
| `crates/marrow/src/cmd_surface.rs`, `crates/marrow/src/cmd_surface/client.rs`, `crates/marrow/src/cmd_surface/client/typescript.rs` | `surface client typescript`: check-style project loading plus TypeScript client rendering over surface ABI/routes, without opening the saved-data store. |
| `crates/marrow/src/cmd_surface.rs`, `crates/marrow/src/cmd_surface/serve.rs` | `surface serve`: foreground loopback HTTP server over descriptor-derived surface routes and operation envelopes; default mode serves read and computed-read routes through `ProjectSurfaceReadSession`, while `--write` also serves create/update/delete/action routes through `ProjectSurfaceSession`. |
| `crates/marrow/src/cmd_surface/serve/cors.rs` | Exact loopback-origin parser and matcher for the opt-in local-browser CORS serving profile. |
