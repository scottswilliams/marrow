...
| `surface serve` | `cmd_surface.rs`, `cmd_surface/serve.rs` | Parses the loopback bind target, opens `ProjectSurfaceReadSession`, derives read routes from `surface.route.v1`, accepts one bounded JSON `POST` per connection, validates route/body operation agreement, dispatches through the read-only surface operation executor, and returns sanitized JSON success/error envelopes. |
...
| `crates/marrow/src/cmd_surface.rs`, `crates/marrow/src/cmd_surface/serve.rs` | `surface serve`: foreground loopback read-only HTTP server over descriptor-derived surface read routes and operation envelopes. |
