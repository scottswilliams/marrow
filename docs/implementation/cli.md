...
- `write_declared_client_if_changed` / `sync_declared_client` — regenerate the project's declared TypeScript client (`config.client`) write-if-changed from the checked program through the single `marrow-json` render owner, comparing the on-disk header digest so a non-surface edit never churns the file. A live `run`, `serve` startup, and `evolve apply` each call `sync_declared_client` in lockstep with the lock they re-project; a `client` set without any surface emits the `config.client_without_surface` warning and writes nothing, and an isolated dry run skips the write entirely. Read-only `check` never writes the client: it only compares the on-disk header digest against the digest the surface would write, treating an absent or behind client as a single stale-or-absent condition — a non-fatal advisory by default, a fatal `check.stale_client` under `--locked`. A project with no `client`, or none with a surface to project, raises no client condition.
