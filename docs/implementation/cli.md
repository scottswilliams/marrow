...
- `read_committed_lock` — read the committed `marrow.lock` projection as a typed value, failing closed on a corrupt lock (`catalog.lock_corrupt`). `check` uses it to bind first-run identity and to report a stale lock by comparing its source digest against the program's, without opening or repairing the store. Under `--locked` an absent lock over a project that has durable shape to lock — any present store file, whether it opens read-only or is recovery-required after a crash — is a fatal `check.lock_missing`, distinct from the present-but-behind `check.stale_lock`; the advisory mode stays silent on absence so a legitimate first run (no store at all) passes.
- `read_accepted_store_catalog` / `read_accepted_store_catalog_lenient` — the durable-state loaders that bind accepted identity from the live store, the sole write-time authority. The strict form is for commands that inspect or write the store; the lenient form is for read-only `check`, returning a tri-state `AcceptedAuthority` — `Readable` (binds the snapshot), `ExistsButUnreadable` (a crashed/locked/corrupt store file that `check` neither repairs nor fails on but whose presence still demands a committed lock under `--locked`), or `Absent` (a first run). A present-but-unreadable store is never collapsed into absent, so a post-crash store cannot masquerade as a first run and pass an absent lock green. The lock remains the first-run anchor whenever no readable snapshot binds.
...
| File | Responsibility |
|---|---|
| `crates/marrow/src/cmd_data.rs` | `data` dispatch, `roots`/`stats`/`dump`, live-or-backup read-target parsing, snapshot pinning, the streaming JSON-array envelope. `stats` and `dump` render the store through the source-derived schema view, so they emit a `data.orphan` advisory on stderr (a count of cells under members the current source no longer declares) while still exiting 0, keeping their reduced output from silently under-reporting intact data. |
