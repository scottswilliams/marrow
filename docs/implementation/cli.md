...
| `crates/marrow/src/cmd_surface.rs`, `crates/marrow/src/cmd_surface/serve.rs` | `surface serve`: foreground loopback HTTP server over descriptor-derived surface routes and operation envelopes; default mode is read-only, while `--write` serves create/update/delete/action routes through `ProjectSurfaceSession`. |
...
| File | Responsibility |
|---|---|
| `crates/marrow-json/src/lib.rs`, `crates/marrow-json/src/surface.rs` | Outbound rendering for `marrow run --format json` entry returns, the saved-key JSON shape reused by trace and integrity tooling, the `store_snapshot` data-stamp shape reused by data inspection, serialized surface ABI DTOs and descriptor-derived route manifests for check output, checked surface read request DTO decode, generated write request DTO decode, action argument/result DTOs and project execution wrappers, thin TypeScript client rendering, and context-aware surface read-result DTOs. It does not define HTTP serving or opaque cursor tokens. |
