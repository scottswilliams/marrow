...
| `crates/marrow/src/cmd_client.rs`, `crates/marrow/src/cmd_client/typescript.rs` | `client typescript`: check-style project loading plus TypeScript client rendering over surface ABI/routes, without opening the saved-data store. |
...
| `crates/marrow-json/src/lib.rs`, `crates/marrow-json/src/surface.rs` | The saved-key JSON shape reused by trace and integrity tooling, the `store_snapshot` data-stamp shape reused by data inspection, serialized surface ABI DTOs and descriptor-derived route manifests for check output, checked surface read/computed-read request DTO decode, generated write request DTO decode, action argument/result DTOs, computed-read result DTOs, project execution wrappers, thin TypeScript client rendering, and context-aware surface read-result DTOs. It does not define HTTP serving or opaque cursor tokens. |
