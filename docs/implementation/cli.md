...
- `read_committed_lock` — read the committed `marrow.lock` projection as a typed value, failing closed on a corrupt lock (`catalog.lock_corrupt`). `check` uses it to bind first-run identity and to report a stale lock by comparing its source digest against the program's, without opening or repairing the store. Under `--locked` an absent lock over a project that has durable shape to lock (the store-bound accepted catalog is `Some`) is a fatal `check.lock_missing`, distinct from the present-but-behind `check.stale_lock`; the advisory mode stays silent on absence so a legitimate first run passes.
- `read_accepted_store_catalog` / `read_accepted_store_catalog_lenient` — the durable-state loaders that bind accepted identity from the live store, the sole write-time authority. The strict form is for commands that inspect or write the store; the lenient form is for read-only `check`, which degrades to no accepted authority (and the lock as the first-run anchor) when the store is absent or cannot be opened read-only, never creating or repairing it.
