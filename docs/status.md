...
| Transactions | One `transaction` block per mutating export, begun at most once on any path; paths that meet agree on whether it has run. Every normal function exit inside it commits, including `try` and `require` failure; a fault before commit rolls the block back. | [Errors and transactions](language/errors-and-transactions.md) |
