...
| Transactions | A mutating export begins its `transaction` region at most once on any path, and paths that meet agree on whether it has run, so separate arms may each hold a block. Every normal function exit inside it commits, including `try` and `require` failure; a fault before commit rolls the block back. | [Errors and transactions](language/errors-and-transactions.md) |
