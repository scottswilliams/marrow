...
| `marrow surface serve` responses and generated-client runtime output | Application egress | Checked application ABI output chosen by a declared `surface` and its boundary profile; not admin inspection, backup, repair, or raw saved-path export. |
