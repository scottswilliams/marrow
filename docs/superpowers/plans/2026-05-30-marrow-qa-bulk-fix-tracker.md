...
|---|---|---:|---|---|---|
| P1 | Parser, formatter, CLI, diagnostics | 13 | Maybe | `feat-defaults`, `cli-doc-migration`, `lsp-check-diagnostics`, `lsp-retire-inrepo`, `enum-binding-index`, `enum-segment-precision` | `algo-collatz#3`, `algo-json-tokenizer#0`, `algo-matrix-multiply#1`, `algo-run-length-encode#1`, `algo-sieve-primes#1`, `app-library-catalog#6`, `app-versioned-cms#6`, `cluster-cli-config-fmt#0`, `cluster-cli-config-fmt#1`, `cluster-cli-config-fmt#3`, `cluster-cli-config-fmt#4`, `cluster-modules-params#3`, `app-versioned-cms#4` |
...
| P10 | Saved storage, indexes, presence | 5 | No | `unkeyed-required-fields`, `saved-walk-cursor`, `element-loop-semantics` | `app-calendar-scheduler#1`, `cluster-indexes#1`, `cluster-sparse-presence#0`, `cluster-sparse-presence#1`, `fuzz-9#1` |
