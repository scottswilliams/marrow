...
- The user-facing data evolution contract says schemas evolve through source changes plus source-native intent, durable identity is recorded automatically by first `run` or `evolve apply`, `preview` proves data obligations, and `apply` consumes the exact preview witness or fails closed (`docs/data-evolution.md:3`, `docs/data-evolution.md:7`).
- The accepted change table distinguishes source-only changes, explicit `default` and `transform`, destructive `retire`, index rebuilds, fail-closed type/key-shape changes, and maintenance/repair paths (`docs/data-evolution.md:20`, `docs/data-evolution.md:32`).
- Renames are explicit catalog decisions. The docs reject best-effort source diffs and migration scripts for preserving identity (`docs/data-evolution.md:131`, `docs/data-evolution.md:147`).
- The accepted catalog is generated, committed metadata, not source syntax. `check` reads it and may propose changes but never writes it; only `run` and `evolve apply` establish or advance durable identity (`docs/data-evolution.md:149`, `docs/data-evolution.md:176`).
- Activation fencing binds store epoch, source digest, and engine profile. v0.1 supports exactly the compiled epoch and schema, with stale or behind binaries failing closed before writes (`docs/data-evolution.md:186`, `docs/data-evolution.md:225`).
- Index rebuilds are derived work: apply rebuilds and stamps in one transaction, and failed rebuilds publish no partial data (`docs/data-evolution.md:241`, `docs/data-evolution.md:261`).
- Repair is not an unchecked bypass. A repair-required witness blocks check/preview/apply; repair is modeled maintenance code verified before and after (`docs/data-evolution.md:282`, `docs/data-evolution.md:296`).
- The language reference says durable identity is invisible catalog infrastructure, assigns opaque IDs, and advances only on `evolve apply`; source spelling alone is not identity (`docs/language/resources-and-storage.md:179`, `docs/language/resources-and-storage.md:187`).
- `evolve` syntax covers `rename`, `default`, `retire`, and `transform`; the docs state that a bare source diff implies nothing about stored data (`docs/language/resources-and-storage.md:613`, `docs/language/resources-and-storage.md:635`).
- Transform is intentionally constrained: it is per-record, pure, old-only, no saved-data reads, no host effects, no same-evolve reads of rewritten members, and every read member must decode under its current type before apply (`docs/language/resources-and-storage.md:643`, `docs/language/resources-and-storage.md:674`).
- The CLI contract says `evolve preview` discharges source, accepted catalog, store snapshot, and engine metadata into an exact witness; `evolve apply` recomputes that witness, requires exact match, gates the activation window, and commits data plus metadata in one transaction (`docs/cli.md:97`, `docs/cli.md:114`).
- The roadmap excludes an ORM layer, separate migration DSL, and hidden database-owned migration ledger (`docs/roadmap/README.md:33`, `docs/roadmap/README.md:40`).
- The Lane 9 doc reports the lane complete and states the essential discipline: no migration scripts, no source-diff identity inference, one proof-discharge pipeline, exact witness apply, fail-closed activation, and split review by invariant (`docs/roadmap/lanes/lane-09-evolution-activation.md:15`, `docs/roadmap/lanes/lane-09-evolution-activation.md:49`).
- ADR evolution 01 accepts "source-native, migrationless evolution" and explicitly rejects a separate migration subsystem, migration DSL, or hidden database ledger (`marrow-decisions/adr/evolution/01-source-native-migrationless-evolution.md:9`, `marrow-decisions/adr/evolution/01-source-native-migrationless-evolution.md:26`).
- ADR evolution 02 defines the lifecycle as `preview -> inspect -> approve or reject -> apply -> verify`, with exact witnesses, drift aborts, exact destructive approvals, bounded compatibility lenses, and epoch windows (`marrow-decisions/adr/evolution/02-evolution-lifecycle-and-obligations.md:11`, `marrow-decisions/adr/evolution/02-evolution-lifecycle-and-obligations.md:48`).
- ADR catalog identity makes the catalog the durable ABI, not a migration ledger. Rename is never inferred from spelling, and apply consumes exact source/catalog/engine/data/count facts (`marrow-decisions/adr/catalog-identity/02-catalog-lifecycle-and-identity-binding.md:18`, `marrow-decisions/adr/catalog-identity/02-catalog-lifecycle-and-identity-binding.md:82`).
- The historical ADR edits strengthen this direction by adding "the access path is the source" and rejecting a below-language cost optimizer (`marrow-decisions/adr/foundations/01-architecture-laws-and-five-layers.md:56`, `marrow-decisions/adr/foundations/01-architecture-laws-and-five-layers.md:75`), and by saying write lowering may only elide provably redundant operations, never choose semantically distinct plans by statistics (`marrow-decisions/adr/storage-engine/02-transactions-commits-and-recovery.md:28`, `marrow-decisions/adr/storage-engine/02-transactions-commits-and-recovery.md:94`).

## 2. Implementation Summary With Crate/Module References

The implementation substantially matches the local vision.

- `crates/marrow-check/src/evolution/mod.rs` exposes a read-only evolution proof surface split into intents, leaf typing, discharge, preview, transform-read resolution, and witness modules. This is the right crate boundary: checker owns classification and proof, not mutation.
- `crates/marrow-check/src/evolution/witness.rs` defines `EvolutionWitness`, digest fields, catalog/store/engine facts, changed root/index IDs, verdicts, and counts. `is_activatable` is false when any verdict is repair or destructive approval.
- `crates/marrow-check/src/evolution/preview.rs` composes a witness from checked source plus live store facts and remains read-only.
- `crates/marrow-check/src/evolution/discharge.rs` is the proof kernel. It classifies store-key shape changes, source roots, absent source entries, transforms, and a default-deny structural backstop in one discharge path (`crates/marrow-check/src/evolution/discharge.rs:68`, `crates/marrow-check/src/evolution/discharge.rs:134`). This supports the "one pipeline" invariant but is large enough to be a future maintainability risk.
- `crates/marrow-check/src/evolution/intents.rs` rejects bare source diff semantics, typechecks defaults/transforms, and enforces transform purity and old-only reads.
- `crates/marrow-check/src/evolution/leaf_type.rs` uses identity-aware tokens rather than source-spelling comparison for stored leaf shape.
- `crates/marrow-check/src/catalog.rs` binds source declarations to accepted catalog metadata, resolves renames and aliases, records pending identities, applies retire/default/transform metadata, and produces catalog proposals. This is the right place for identity recording semantics because the store and CLI should not rediscover identity.
- `crates/marrow-check/src/lib.rs` has the production catalog writer discipline: `commit_pending_identity` writes only an initial baseline and never auto-advances an accepted catalog; `write_accepted_catalog` is the single production catalog writer with atomic file replacement (`crates/marrow-check/src/lib.rs:332`, `crates/marrow-check/src/lib.rs:386`).
- `crates/marrow-run/src/evolution/apply.rs` validates the exact witness, fences against the pre-apply store shape, gates repair/destructive approval, stages verdict work, checks staged counts against witness counts, stamps metadata, and commits atomically (`crates/marrow-run/src/evolution/apply.rs:84`, `crates/marrow-run/src/evolution/apply.rs:216`).
- `crates/marrow-run/src/evolution/validate.rs` recomputes preview and requires full witness equality before apply.
- `crates/marrow-run/src/evolution/window.rs` implements the v0.1 exact epoch/source-digest/engine-profile activation fence.
- `crates/marrow-run/src/evolution/admission.rs` admits only activatable witnesses, requires maintenance for destructive retire, and matches approval by exact catalog ID and populated count.
- `crates/marrow-run/src/evolution/backfill.rs` stages default backfills, index rebuilds/drops, and retires from checked places and live store scans, not from an opaque migration list.
- `crates/marrow-run/src/evolution/transform.rs` uses runtime evaluation and existing codecs rather than a second transform interpreter, which lowers duplicate-semantics risk.
- `crates/marrow/src/cmd_evolve/mod.rs` keeps the CLI surface to `preview` and `apply`, reuses preview for data-attached checks, commits pending identity first, applies the witness, writes the accepted catalog after the store commit, and handles the crash window by exact resume.
- `crates/marrow/src/cmd_run.rs` records baseline identity before durable run and fences persistent stores before executing.

Test coverage is also aligned with the design:

- `crates/marrow-check/tests/evolution_discharge.rs` exercises source-driven proof cases for defaults, renames, enum changes, structural/key-shape failure, index obligations, transforms, and repair verdicts through the check pipeline.
- `crates/marrow-run/tests/evolution_apply.rs` explicitly says every drift dimension is exercised by mutating the witness or store and proving apply aborts before staging a write (`crates/marrow-run/tests/evolution_apply.rs:1`, `crates/marrow-run/tests/evolution_apply.rs:9`). Test names cover source digest drift, transform digest drift, store commit drift, stale writer fencing, engine profile drift, destructive approval, maintenance, rollback, and idempotence.
- `crates/marrow/tests/evolve_cli.rs` covers preview rendering, apply consuming a witness, repair rejection, destructive approval rendering, accepted catalog/store lockstep for retire and rename, half-applied resume, divergent resume fail-closed, noop apply, and absence of legacy `evolve migrate` subcommands.
...
Weak Rust shape risk: moderate. `crates/marrow-check/src/evolution/discharge.rs` is 2572 lines and `crates/marrow-check/src/evolution/intents.rs` is 1001 lines. They appear cohesive, but the size is already close to the point where future changes could create catch-all dispatch, duplicate classifiers, and comment sediment. This is not a reason to reverse the architecture, but it is a foundation risk before more evolution cases are added.
...
No optimizer / access path as source risk: coherent with recent ADR edits. The historical ADR edits make migrationless evolution more plausible because evolution proofs can name the source-forced access path and bounded scans instead of reverse-engineering optimizer choices. The risk is developer ergonomics: source must make expensive scans explicit and explainable.
