...
| Message-parsing logic | L03 syntax, L04 schema, L05 project-model, L12 store, and L13 backup/restore have no `message.contains` semantic assertions after integration. L06 schema-payload, duplicate-root, and duplicate-declaration slices migrated the checker assertions they touched; remaining checker/runtime/tooling areas still need lane-local migration. | needs-lane | L06-L11, L14 |
...
- `crates/marrow-check/src/analysis.rs` - status: in-lane; owner: L06 checker-core; notes: duplicate root owner payload slice integrated; broader checker-core review remains.
...
- `crates/marrow-check/src/checks.rs` - status: in-lane; owner: L06 checker-core; notes: schema diagnostic, duplicate-root, and duplicate-declaration payload suppression matches integrated; broader checker-core review remains.
...
- `crates/marrow-check/src/lib.rs` - status: in-lane; owner: L06 checker-core; notes: schema diagnostic, duplicate-root, and duplicate-declaration payload variants integrated; broader checker-core review remains.
...
- `crates/marrow-check/tests/project.rs` - status: in-lane; owner: L06 checker-core; notes: early schema-family, duplicate-root, and duplicate-declaration assertions now use exact diagnostic payloads for the integrated slices; broader checker-core test cleanup remains.
