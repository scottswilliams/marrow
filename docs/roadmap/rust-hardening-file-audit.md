...
- `crates/marrow/src/cmd_evolve/store.rs` - status: blocked; owner: L14 cli-tools-server; notes: B007 found `evolve preview --format json` store-open failures render as text through the read-only opener; source fix requires dirty sibling files.
...
- `crates/marrow/src/main.rs` - status: blocked; owner: L14 cli-tools-server; notes: B007 requires making the shared read-only store inspection error path format-aware; blocked while this file is staged in `/Users/scottwilliams/Dev/marrow-engine-resident-catalog`.
