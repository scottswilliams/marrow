...
| Message-parsing logic | L03 syntax, L04 schema, L05 project-model, L12 store, and L13 backup/restore have no `message.contains` semantic assertions after integration. L06 schema-payload, duplicate-root, duplicate-declaration, duplicate-module, module-path, rejected-surface, and schema-unsupported-map slices migrated the checker assertions they touched; remaining checker/runtime/tooling areas still need lane-local migration. | needs-lane | L06-L11, L14 |
...
- `crates/marrow-check/tests/project.rs` - status: in-lane; owner: L06 checker-core; notes: early schema-family, duplicate-root, duplicate-declaration, duplicate-module, module-path, rejected-surface, and schema unsupported-map assertions now use exact diagnostic payloads for the integrated slices; broader checker-core test cleanup remains.
