...
| L14 cli-tools-server | `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-cli-tools-server`; trace slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-trace-cli`; usage/v01 slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-usage-v01`; fmt/test slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-fmt-test-cli`; LSP slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-lsp`; trace-source slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-trace-source`; cmd-fmt source slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-cmd-fmt-source`; dry-run source blocker slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-dry-run-source`; serve-protocol/base64 slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-serve-protocol-source`; evolve args/store slice `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store` | latest lane `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store`; latest reviews `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-review-soundness-evolve-args-store` and `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-review-idiom-evolve-args-store`; latest main `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store-integration` | initial CLI slice `13852686ed8317e1b567f941ff160de345738d3b`; trace CLI source `4a3e473bbf6a9465c46e8d0fd05f9966e1fe2f1a`; usage/v01 no-source base `d732ad3a0d395a713f741c8882e0111748763284`; fmt/test source base `c1e97e1986d736377e0b0e4e2e2e146c5d2747e5`; LSP source base `3d4929a2ab199b503f8534526e17260d65f097cb`; trace-source no-source base `de9c2f52a5e72d1c808c876ef9cf2c6952a20d62`; cmd-fmt source base `103d39a446ecb26d6e4b4d6c051b272c5b02ad15`; dry-run source blocker base `40767f054cb522af74062d9ac27665a13225715a`; serve-protocol/base64 base `451df5e4256dde7a04a1a015930a99e8fa348fdb`; evolve args/store base `c375e57097b8a28880f13a2820c94f204b80b84c` | latest source/main `380e864f417b81cd081969a7364517144b63ae0b`; dry-run blocker produced no source commit | in-lane | focused CLI, trace, usage, v0.1, fmt, test, LSP, cmd-fmt, dry-run, serve-protocol, base64, evolve-args, `marrow-run`, `marrow`, workspace build/test, workspace clippy, and fmt gates passed for integrated slices | evolve args/store review failed on preview JSON store-open text rendering and passed parser re-review after typed command fix; store issue deferred to B007; serve-protocol soundness failed on oversized request-line suffix parsing and non-canonical base64 pad bits; base64 pass after fix and request-line issue deferred to B006; dry-run source failed on mixed program stdout with JSONL tooling records and overstrong byte-for-byte dry-run wording; cmd-fmt source failed on write failures reported as `io.read`, then pass after fix; LSP slice failed on unbounded header-line/header-block parsing and missing checker diagnostic range assertion, then pass after fix; trace-source no-source review pass; otherwise pass, no findings after review-fixes | evolve args/store idiom/spec failed on boolean parser mode and duplicate preview rejection, then passed after typed command fix while leaving store formatting deferred to B007; serve-protocol/base64 idiom/spec pass after fix; dry-run idiom/spec failed on structured target assertions and typed-target doc wording; cmd-fmt idiom/spec pass before and after source fix; trace-source no-source review pass; LSP idiom/spec pass after source fix; fmt/test slice failed on multi-file fmt coverage, OS-prose negative assertion, and low-value comments, then pass after fix; trace slice failed on JSONL order assertion gap, then pass after fix; prior diagnostic slice failed on semantic text assertions and duplicate JSON parsing, then pass after fix | L14-R001, L14-R002, trace-order finding, L14-IDIOM-001 through L14-IDIOM-003, LSP-001, LSP-002, cmd-fmt write-error finding, base64 pad-bit finding, and evolve parser mode finding fixed and re-reviewed; dry-run findings deferred to B005; serve request-line finding deferred to B006 because `serve/mod.rs` is staged in the active `marrow-engine-resident-catalog` worktree; evolve preview store-open formatting deferred to B007 because required files are dirty/unavailable there (`cmd_evolve/mod.rs` and `main.rs` staged, `tests/evolve_cli.rs` unmerged); usage/v01 and trace-source no-source reviews had no findings | CLI diagnostic test-support, trace JSONL assertion, fmt/test coverage cleanup, LSP header-bound/range, cmd-fmt `io.write`, shared base64 canonical decoder, and evolve parser typed-mode slices integrated; dry-run source slice retired without integration; serve connection framing and evolve preview store-open formatting blocked; usage/v01 and trace-source reviewed-clean with no source change; untouched L14 sibling source and CLI files remain unreviewed |
...
- `crates/marrow/src/cmd_evolve/args.rs` - status: complete; owner: L14 cli-tools-server; notes: reviewed and integrated typed `Command` parser mode; duplicate preview post-check and boolean parser mode removed; focused evolve CLI, `marrow`, workspace build/test/clippy, formatter, LSP, and absence scans passed.
- `crates/marrow/src/cmd_evolve/mod.rs` - status: blocked; owner: L14 cli-tools-server; notes: B007 requires threading preview format into read-only store-open errors; blocked while this file is staged in `/Users/scottwilliams/Dev/marrow-engine-resident-catalog`.
- `crates/marrow/src/cmd_evolve/render.rs` - status: unreviewed; owner: L14 cli-tools-server; notes: initial inventory.
- `crates/marrow/src/cmd_evolve/store.rs` - status: complete; owner: R1 release-hardening; notes: B007 resolved in Wave 3 R1 — evolve preview store-open failures now render through the selected CheckFormat, not hard-coded text.
...
- `crates/marrow/src/cmd_run.rs` - status: complete; owner: R1 release-hardening; notes: B005 resolved in Wave 3 R1 — `--dry-run`/`--trace` tooling reports go to stderr, program output stays on stdout; a `print(...)` + `--dry-run --trace --format jsonl` regression pins the separation.
- `crates/marrow/src/cmd_test.rs` - status: unreviewed; owner: L14 cli-tools-server; notes: initial inventory.
- `crates/marrow/src/dry_run.rs` - status: complete; owner: R1 release-hardening; notes: B005 resolved in Wave 3 R1 — byte-for-byte wording narrowed to logical saved-data stability (the same records read back), with the test renamed and reworded accordingly.
...
- `crates/marrow/src/main.rs` - status: complete; owner: R1 release-hardening; notes: B007 resolved in Wave 3 R1 — the shared read-only store-inspection error path is format-aware, and resolve_store_path/open_run_store thread the selected format on the write path too. Residual B17: report_io_error still labels catalog-write failures `io.read` (backlog).
- `crates/marrow/src/serve/mod.rs` - status: complete; owner: R1 release-hardening; notes: B006 resolved in Wave 3 R1 — an oversized request line is drained through its newline or closes the connection, so its suffix can no longer be parsed as a second request; a stalled-mid-drain repro (StallingReader) pins it.
...
- B005 (RESOLVED, Wave 3 R1): program stdout is kept separate from JSON/JSONL trace and dry-run tooling reports (reports to stderr); a `print(...)` + `--dry-run --trace --format jsonl` regression pins it; dry-run wording narrowed from native-file byte identity to logical saved-data stability.
- B006 (RESOLVED, Wave 3 R1): an over-limit request line is drained through its newline or closes the connection, so the over-limit frame's suffix can no longer be parsed as a second request; a stalled-mid-drain repro (StallingReader) plus an oversized-line-then-valid-request repro pin it.
- B007 (RESOLVED, Wave 3 R1): read-only store-open failures render through the selected `CheckFormat` (not hard-coded text), and the write-path resolver threads the format too; a catalog-write-failure regression covers the selected structured format. Residual B17 (io.read vs io.write code precision) remains backlog.
- B008 (RESOLVED, Wave 2): the checker statically validates `Error(...)` field argument types — non-string `code`/`message`/`help` are rejected before runtime via `check.call_argument`, open `data` preserved; coverage asserts typed checker diagnostics (checked_program_error_construct.rs), and three runtime tests migrated to assert the checker rejection.
...
## L14 Evolve Args And Preview Store Evidence

- Worktree: `/Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store`.
- Target dirs:
  - Lane: `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store`.
  - Soundness review: `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-review-soundness-evolve-args-store`.
  - Idiom/spec review: `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-review-idiom-evolve-args-store`.
  - Integration: `/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store-integration`.
- Base/head:
  - Lane base: `c375e57097b8a28880f13a2820c94f204b80b84c`.
  - Source and main commit: `380e864f417b81cd081969a7364517144b63ae0b`.
- Changed files:
  - `crates/marrow/src/cmd_evolve/args.rs`
- Reviewed files and context:
  - Owned: `crates/marrow/src/cmd_evolve/args.rs` and `crates/marrow/src/cmd_evolve/store.rs`.
  - Context: `crates/marrow/src/cmd_evolve/mod.rs`, `crates/marrow/src/main.rs`, and `crates/marrow/tests/evolve_cli.rs`.
- Baseline and focused gates:
  - Before source edits, `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store cargo test --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml -p marrow --test evolve_cli`, formatter, clippy, `cargo test -p marrow`, LSP diagnostics, and absence scans passed with no source diff.
  - After the parser fix, `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store cargo fmt --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml --all -- --check` passed with no output.
  - `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store cargo test --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml -p marrow --test evolve_cli -- --nocapture` passed with 19 tests.
  - `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store cargo clippy --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml -p marrow --all-targets -- -D warnings` passed.
  - `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store cargo test --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml -p marrow` passed.
  - Rust LSP diagnostics for `args.rs` and `store.rs` were clean.
- Source review:
  - Initial soundness review: fail. Reviewer reproduced that `marrow evolve preview --format json` reports native store-open errors as text because `preview_store` delegates to a read-only opener that uses `CheckFormat::Text`; parser probes otherwise passed.
  - Initial idiom/spec review: fail. Reviewer found a boolean parser mode, `allow_apply_flags`, plus a duplicate preview post-check for apply-only flags.
  - Source fix: `args.rs` now uses a private typed `Command` enum for the common parser, accepts/rejects apply-only flags in one parser match, and removes the duplicate preview post-check.
  - Soundness re-review: pass for the changed parser path. Reviewer rebuilt the CLI and reran parser probes for preview/apply flag acceptance, duplicate and missing formats, approval id/count boundaries, project-dir arity, unknown options, and help paths.
  - Idiom/spec re-review: pass for the parser shape. Reviewer verified the typed parser mode, narrow `pub(super)` surfaces, no manifest churn, no source-smell matches, and clean LSP diagnostics.
- Deferred blocker:
  - The preview store-open format finding is tracked as B007 because `git -C /Users/scottwilliams/Dev/marrow-engine-resident-catalog status --short -- crates/marrow/src/cmd_evolve/mod.rs crates/marrow/src/main.rs crates/marrow/tests/evolve_cli.rs` reported `M  crates/marrow/src/cmd_evolve/mod.rs`, `M  crates/marrow/src/main.rs`, and `UU crates/marrow/tests/evolve_cli.rs`.
  - This slice did not edit `store.rs`, `mod.rs`, `main.rs`, or `evolve_cli.rs`; `store.rs` remains blocked until the format-aware read-only open path and regression can be owned file-disjointly.
- Absence and sibling scans:
  - `git diff --check` passed with no output.
  - `git diff --name-only -- Cargo.lock ':(glob)**/Cargo.toml'` returned no output.
  - `rg -n 'allow_apply_flags|common\([^\n]*false|common\([^\n]*true|maintenance \|\| parsed\.approval|fn preview_common|fn apply_common|ApplyCommonArgs|CommonParser' crates/marrow/src/cmd_evolve/args.rs` returned no matches.
  - `rg -n '\bunsafe\b|message\.contains|stderr\.contains|stdout\.contains|error\.to_string\(\)\.contains|assert!\([^\n]*contains|legacy|prototype|compat|temporary|TODO|FIXME|fallback|bridge|raw' crates/marrow/src/cmd_evolve/args.rs crates/marrow/src/cmd_evolve/store.rs` returned no matches.
- Integration gates:
  - Before integration, local `main` and `origin/main` were at `c375e57097b8a28880f13a2820c94f204b80b84c`; the lane rebased cleanly and `main` fast-forwarded to `380e864f417b81cd081969a7364517144b63ae0b`.
  - On the rebased lane, `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store-integration cargo fmt --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml --all -- --check` passed with no output.
  - `rg -n '\bunsafe\b' --glob '*.rs' /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store` returned no matches through a wrapper that treats no matches as success.
  - `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store-integration cargo build --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml --workspace` passed.
  - `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store-integration cargo test --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml --workspace` passed.
  - `CARGO_TARGET_DIR=/Users/scottwilliams/Dev/.build/marrow-targets/rust-hardening-l14-evolve-args-store-integration cargo clippy --manifest-path /Users/scottwilliams/Dev/marrow-rust-hardening-l14-evolve-args-store/Cargo.toml --workspace --all-targets -- -D warnings` passed.
  - `git -C /Users/scottwilliams/Dev/marrow push origin main` pushed `c375e57..380e864`.
