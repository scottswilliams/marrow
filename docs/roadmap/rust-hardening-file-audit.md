...
| Message-parsing logic | L03 syntax, L04 schema, L05 project-model, L12 store, and L13 backup/restore have no `message.contains` semantic assertions after integration. L06 schema-payload and duplicate-root slices migrated the checker assertions they touched; remaining checker/runtime/tooling areas still need lane-local migration. | needs-lane | L06-L11, L14 |
...
- `crates/marrow-check/src/checks.rs` - status: in-lane; owner: L06 checker-core; notes: schema diagnostic and duplicate-root payload suppression matches integrated; broader checker-core review remains.
...
- `crates/marrow-check/src/lib.rs` - status: in-lane; owner: L06 checker-core; notes: schema diagnostic and duplicate-root payload variants integrated; broader checker-core review remains.
...
- `crates/marrow-check/tests/project.rs` - status: in-lane; owner: L06 checker-core; notes: early schema-family and duplicate-root assertions now use exact diagnostic payloads for the integrated slices; broader checker-core test cleanup remains.
