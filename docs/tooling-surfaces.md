...
| `marrow surface serve` | Local application-surface serving. | Foreground loopback HTTP server over descriptor-derived `/surface/v1/{read\|update\|action}/<operation-tag>` routes and `surface.operation.v1` request envelopes. Default mode opens `ProjectSurfaceReadSession` and serves read routes only; `--write` opens `ProjectSurfaceSession` and also serves sparse-update/action routes under the native single-owner writer lock. It serves JSON `POST` only, enforces exact route/body operation agreement, can opt into CORS for one exact loopback browser origin with `--cors-origin`, returns sanitized `surface.*` error envelopes, processes at most one request per connection, and closes every response. It is not remote serving, authentication, generated clients, opaque cursor tokens, create/delete CRUD, store creation, catalog repair, maintenance, or a raw saved-data API. |
