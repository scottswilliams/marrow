...
- The only local application API is the checked-surface profile exposed by
  `marrow surface serve`. Future remote, generated-client, or opaque cursor APIs
  must be generated from the same shared checked facts, not promoted from raw
  bytes, raw paths, or an ad hoc saved-data access language.
