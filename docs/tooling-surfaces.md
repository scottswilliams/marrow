...
| `marrow surface serve` | Local read-only application-surface serving. | Foreground loopback HTTP server over descriptor-derived `/surface/v1/read/<operation-tag>` routes and `surface.operation.v1` request envelopes. It opens `ProjectSurfaceReadSession`, serves JSON `POST` only, enforces exact route/body operation agreement, returns sanitized `surface.*` error envelopes, processes at most one request per connection, and closes every response. It is not remote serving, authentication, generated clients, opaque cursor tokens, create/delete CRUD, writable update/action serving, store creation, catalog repair, maintenance, or a raw saved-data API. |
...
- The only local application API is the checked-surface read profile exposed by
  `marrow surface serve`. Future writable, remote, generated-client, or opaque
  cursor APIs must be generated from the same shared checked facts, not promoted
  from raw bytes, raw paths, or an ad hoc saved-data access language.
