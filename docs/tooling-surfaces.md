...
| `marrow-json::surface` DTOs | Checked application read/update JSON. | Renders serialized surface ABI descriptors for `marrow check --format json|jsonl`, decodes checked read request parameters through admitted runtime reads, decodes sparse update request DTOs through admitted runtime updates, and serializes already-executed surface records, pages, typed values, catalog identities, and context-aware typed cursor boundaries. It is not HTTP routing, opaque cursor tokens, generated clients, create/delete body decode, or a raw saved-data access API. |
