...
| `marrow-json::surface` DTOs | Checked application-read JSON. | Decodes checked read request parameters through admitted runtime reads and serializes already-executed surface records, pages, typed values, catalog identities, and context-aware typed cursor boundaries. It is not HTTP routing, opaque cursor tokens, generated clients, write-body decode, or a raw saved-data access API. |
