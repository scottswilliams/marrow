...
| Checked `SurfaceFact` / future `SurfaceAbi` facts | Unstable checked application facts. | Resolves declared surfaces to checker-valid store, member, and index identities. These facts are transport-neutral inputs for future boundary profiles; they are not HTTP routes, JSON shapes, TypeScript names, raw saved paths, or an admin query API. Their typed catalog status reports whether stable client export is blocked by a pending catalog proposal or missing accepted catalog IDs. |
