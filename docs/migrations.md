...
field, or rebuilds an index, that work is ordinary `.mw` code you run, run by a
tool, and inspect afterward. There is no migration DSL and no hidden migration
ledger in the database.

This page covers what each schema change requires today: adding sparse vs
required elements, tracking renames with `@id`, populating required fields,
backfilling indexes, the `--maintenance` capability, repair, and restore policy.
...
[Data Modeling](data-modeling.md) is the guide to shaping it. This page picks up
where modeling leaves off: changing a schema that already has saved data.
...
- `marrow data integrity` does **not** report the gap. It verifies that every
  stored value *decodes* as its declared type (`data.decode`) and flags data
  under unknown members (`data.orphan`); it does not check required-field
  completeness across records.
...
- It does **not** move saved data. After a rename, the migration still decides
  what to read, rewrite, or delete. The ID gives tools a durable handle; it does
...
- **Required-field delete.** `delete ^books(id).title` removes a required field
  in place. Outside maintenance this is rejected; you would delete the whole
  record instead.
- **Raw quoted-segment access.** `^books(id)."oldTitle"` reads and writes a
  literal backend segment, bypassing the schema's declared fields and index
  maintenance. This is the door to raw data the schema does not model — legacy
  fields from before a rename, import staging, repair. A raw write takes a
  string (raw segments are an untyped text boundary; a non-string scalar is a
  `run.type` error). A raw read of an absent segment is a catchable
  absent-element error.
...
`marrow restore ./project <archive>` replays an archive into an **empty** store,
in one transaction (the target gains the whole archive or is left unchanged).
...
Replace, merge, and repair restores — the non-empty cases — are **deferred**.
They are not implemented. When they land they will be explicit maintenance
actions routed through the maintenance capability, not a relaxation of the
empty-target guard.
