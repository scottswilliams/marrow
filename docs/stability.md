...
| Saved-data identity | The live store is the authority for accepted saved-data identity, projected to the committed `marrow.lock` documented in [data-evolution.md](data-evolution.md). The lock's per-entry stable IDs, lifecycle, canonical paths with aliases, and shape fingerprints, plus its append-only ledger of retired and reserved IDs, are the v0.1 identity ABI. Future id-allocation policy flows through shape evolution, not ad hoc fields accepted by v0.1 readers. |
