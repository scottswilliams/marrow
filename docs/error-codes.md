...
| `check.rejected_surface` | Source uses a parsed construct outside the accepted v0.1 surface, such as saved-path `inout` call arguments or old saved traversal method shapers such as `.take(...)`, `.window(...)`, and `.resume(...)`. Reserved syntax forms such as `merge`, `lock`, `out`, `@id`, and `~` are parser diagnostics instead. |
