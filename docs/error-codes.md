...
| Code | Meaning |
|---|---|
| `cli.command_unsupported` | A command name is recognized but not yet available on this beta line: its owning capability is being refounded and returns through a later lane. `marrow init`, `marrow fmt`, `marrow check`, `marrow run`, `marrow test`, and `marrow client typescript` are the currently available commands. |
