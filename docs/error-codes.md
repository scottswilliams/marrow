...
| `check.sequence_position` | A write to a sequence position the spec proves addresses no node: a statically-known zero or negative position in a single integer-keyed layer, which is 1-based. A guarded read of such a position resolves to absent at run time and is not flagged. The static counterpart of the absent fault a dynamic non-positive position raises. |
...
| `check.lock_missing` | A `marrow check --locked` failure for CI: the committed `marrow.lock` is absent over a project that has durable shape to lock (a stamped store / accepted catalog), so the gate fails closed rather than passing a project whose lock was never committed or was deleted. Distinct from `check.stale_lock`, which reports a present-but-behind lock. A legitimate first run, which has no durable shape to lock yet, raises no condition: an absent lock there is expected and `--locked` still passes. |
