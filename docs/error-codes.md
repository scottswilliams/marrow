...
| `check.transaction_empty` | An export owns a `transaction` block but performs no durable operation, directly or through a call, so it opens no store session. Remove the block or add the durable work the export is meant to perform. The requirement applies to the entire export: a read before an empty block satisfies it. |
