...
| `check.transaction_reopened` | A mutating export opens a second `transaction` block. An export owns one block, begins it at most once and commits on every normal exit after begin. Combine the durable work into one block. |
| `check.transaction_uncommitted` | A path returns while its `transaction` region is still open. Every normal exit from an entered region must commit its staged writes after evaluating the exit value and before returning. |
...
| `check.transaction_misplaced` | A `transaction` block appears in a helper that is not `pub` or in a `test` body. Only an export owns a block: a helper runs inside its caller's block, and a test invokes ordinary functions. Move the block to the export that owns the durable work. |
...
| `store.apply_unsupported` | Explicit apply requires unchanged old durable representations and permits only sparse scalar field additions. The selected change is outside that scope; no store metadata was published. |
